...
const generateCoffeeMoistureData = () => {
  const normal = createNormalGenerator(1401);
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const moisture = normal(mean, std);
...
const generateCoffeeDefectsData = () => {
  const normal = createNormalGenerator(1402);
  const data: Record<string, unknown>[] = [];
  ...
  for (let i = 0; i < 150; i++) {
    ...
    const defects = Math.max(0, Math.round(normal(meanDefects, i < 100 ? 2 : 8)));
