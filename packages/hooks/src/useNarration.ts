...
export function useNarration(options: UseNarrationOptions): UseNarrationReturn {
  ...
  const doFetch = useCallback(
    async (ctx: AIContext) => {
      ...
      try {
        // Tracing is handled in the service layer (aiService.ts)
  ...
  useEffect(() => {
    if (!context || !fetchNarration) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- responding to context/fetchNarration becoming null
      setStatus('idle');
      return;
    }
