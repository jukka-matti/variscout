...
import type { UseHypothesesReturn } from './useHypotheses';
...
export interface UseQuestionGenerationOptions {
  ...
  /** Factor column names */
  factors: string[];
  /** Hypotheses state (for generateInitialQuestions + question click) */
  hypothesesState: Pick<
    UseHypothesesReturn,
    'hypotheses' | 'generateInitialQuestions' | 'setFocusedQuestion'
  >;
...
export interface UseQuestionGenerationReturn {
  ...
  /** Questions generated from Factor Intelligence (Hypothesis objects with questionSource) */
 ...
 * and persisted as Hypothesis objects via generateInitialQuestions().
 *
...
export function useQuestionGeneration({
  ...
  // Extract stable references from hypothesesState to avoid object-identity deps
  const { hypotheses, generateInitialQuestions } = hypothesesState;
  ...
  useEffect(() => {
    ...
    const existingQuestions = hypotheses.filter(h => h.questionSource === 'factor-intel');
  ...
  useEffect(() => {
    ...
    // and "interact" for L3).
    const supportedL1 = hypotheses.filter(
      h =>
        h.questionSource === 'factor-intel' &&
        h.status === 'supported' &&
        !spawnedFollowUpsRef.current.has(h.id)
    );
    ...
    const newFollowUps = followUps.filter(q => !isFollowUpAlreadyPresent(q, hypotheses));
  ...
  const handleQuestionClick = useCallback(
    (question: Hypothesis) => {
      ...
      hypothesesState.setFocusedQuestion(question.id);
    },
    [hypothesesState]
  );

  // Derive the current questions from hypotheses (factor-intel sourced)
  const questions = useMemo(
    () => hypotheses.filter(h => h.questionSource === 'factor-intel'),
    [hypotheses]
  );
...
/**
 * Check if a follow-up question already exists in the hypotheses list.
 * Uses factor name(s) and question type for deduplication rather than text matching.
 */
function isFollowUpAlreadyPresent(q: GeneratedQuestion, hypotheses: Hypothesis[]): boolean {
  const factorIntel = hypotheses.filter(h => h.questionSource === 'factor-intel');
  ...
  if (q.type === 'main-effect') {
    ...
    return factorIntel.some(h => h.factor === q.factors[0] && h.level != null);
  }
  ...
  if (q.type === 'interaction') {
    ...
    return factorIntel.some(
      h => h.factor === q.factors[0] && h.evidence?.etaSquared != null // Interaction questions use deltaR² stored as etaSquared
    );
  }
