...
describe('detectEntryScenario', () => {
  ...
  it('returns "routine" when no context fields', () => {
    expect(detectEntryScenario({})).toBe('routine');
  });

  it('returns "problem" when no processContext at all', () => {
    expect(detectEntryScenario()).toBe('problem');
  });
});
