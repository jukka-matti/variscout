...
describe('useBoxplotData', () => {
  ...
  it('computes correct groups for simple data (2 groups, 3 values each)', () => {
    ...
    const groupA = result.current.find(g => g.key === 'A');
    const groupB = result.current.find(g => g.key === 'B');
  ...
  it('handles single group', () => {
    ...
    expect(result.current[0].key).toBe('X');
    expect(result.current[0].values).toEqual([5, 15, 25, 35, 45]);
    expect(result.current[0].mean).toBeCloseTo(25, 5);
  });
  ...
  it('filters out NaN values', () => {
    ...
    expect(result.current[0].values).toEqual([10, 30, 50]);
    expect(result.current[0].mean).toBeCloseTo(30, 5);
  });
