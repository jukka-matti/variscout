...
- Persistence: the analyze snapshot — `findings`, `categories`, `hypotheses`, `causalLinks`, `scopes` — serializes via the `DocumentSnapshot.analyze` facet through `useProjectActions` into `.vrs` (Apr 2026 fix). New analyze entities also need `apps/azure/src/db/schema.ts` + `useEditorDataFlow.ts` updates. `hydrateDocumentSnapshot` strict-asserts via `validateDocumentSnapshot` (PO-8a): version mismatch and corrupt shape throw typed errors (`DocumentSnapshotVersionMismatchError` / `DocumentSnapshotCorruptError`) — validation is deliberately SHALLOW (structural only, no field-level assertions; ADR-091). `viewState` is per-user session state: it lives in `ProjectState` but NEVER serializes into `DocumentSnapshot`, and `setViewState` does not mark unsaved (PO-8a strip).
