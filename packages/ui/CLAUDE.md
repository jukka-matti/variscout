...
- Error service (`errorService`) and hooks (`useTheme`, `useGlossary`) are also exported from @variscout/ui. (`useIsMobile`/`BREAKPOINTS` removed — desktop-only per ADR-093 D3.)
- `Canvas` is the canonical canvas implementation for FRAME / Process Hub work. `LayeredProcessViewWithCapability` is a deprecated compatibility wrapper; new work targets `Canvas` or `CanvasWorkspace`. Lens × level matrix (8f spec §10) covers L1 outcome / L2 process / L3 author across the lens dimensions; followup work tracked on `canvas-viewport-8f-followups`. **Wall single home:** the Analyze-tab Wall is the only `WallCanvas` mount (`CanvasWallOverlay` retired by CS-12 2026-06-04; mobile reaches the Wall via `WallShortcutButton`, gated by `useHasAnalyzeContent`). `LocalMechanismView` is step-local-only (mini-charts + η² rankings + quick-action modal; no embedded Wall or EvidenceMapBase).
