...
describe('CoScoutPanelBase', () => {
  ...
  describe('typed input', () => {
    it('keeps the typed draft editable until the user sends it', () => {
      const onSend = vi.fn();
      render(<CoScoutPanelBase {...defaultProps} onSend={onSend} />);

      const input = screen.getByTestId('coscout-input');
      fireEvent.change(input, { target: { value: 'Typed draft question' } });

      expect(input).toHaveValue('Typed draft question');
      expect(onSend).not.toHaveBeenCalled();

      fireEvent.click(screen.getByLabelText('coscout.send'));

      expect(onSend).toHaveBeenCalledWith('Typed draft question');
    });
  });
});
