...
export function ChipRailItem({
  ...
  return (
    <button
      ...
      className="flex w-full items-center justify-between gap-3 rounded-md border border-edge bg-surface-primary px-3 py-2 text-left text-sm text-content shadow-sm transition-colors hover:bg-surface-tertiary focus:outline-none focus:ring-2 focus:ring-status-info/50 disabled:cursor-not-allowed disabled:opacity-60 data-[dragging=true]:opacity-60"
      style={style}
      {...attributes}
      {...listeners}
      onKeyDown={event => {
        if (disabled || !onKeyboardPickUp) return;
