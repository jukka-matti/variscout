...
export const DataQualityBanner: React.FC<DataQualityBannerProps> = ({
  ...
  return (
    <div
      className="bg-surface-secondary border border-edge rounded-xl overflow-hidden"
      role="status"
    ...
    >
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-content">{tf('quality.rowsReady', { count: validRows })}</span>
        </div>
      ...
      {showActions && (onViewExcludedRows || onViewAllData || onContinue) && (
        <div className="flex items-center gap-3 p-4 border-t border-edge bg-surface-tertiary/50">
          ...
          {onViewAllData && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-surface-tertiary hover:bg-surface-elevated text-content rounded-lg transition-colors"
            >
              <Table size={16} />
              {t('quality.viewAll')}
            </button>
