...
// ============================================================================
// Time-based subgrouping
// ============================================================================

describe('groupDataIntoSubgroups — time-interval method', () => {
  const makeTimeRows = (entries: { time: string; value: number }[]): DataRow[] =>
    entries.map(e => ({ Weight: e.value, Timestamp: e.time }));

  it('groups by hour buckets', () => {
    const rows = makeTimeRows([
      { time: '2025-01-15T14:10:00', value: 10.0 },
      { time: '2025-01-15T14:40:00', value: 10.2 },
      { time: '2025-01-15T15:05:00', value: 10.1 },
      { time: '2025-01-15T15:30:00', value: 10.3 },
    ]);
    ...
    expect(result).toHaveLength(2);
    expect(result[0].label).toBe('Jan 15 14:00');
    expect(result[0].values).toEqual([10.0, 10.2]);
    expect(result[1].label).toBe('Jan 15 15:00');
    expect(result[1].values).toEqual([10.1, 10.3]);
  });

  it('groups by day buckets', () => {
    const rows = makeTimeRows([
      { time: '2025-01-15T08:00:00', value: 10.0 },
      { time: '2025-01-15T16:00:00', value: 10.2 },
      { time: '2025-01-16T09:00:00', value: 10.1 },
    ]);
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    expect(result[0].values).toEqual([10.0, 10.2]);
    expect(result[1].label).toBe('Jan 16');
    expect(result[1].values).toEqual([10.1]);
  });

  it('groups by week buckets', () => {
    const rows = makeTimeRows([
      { time: '2025-01-06T10:00:00', value: 10.0 }, // W02
      { time: '2025-01-08T10:00:00', value: 10.1 }, // W02
      { time: '2025-01-13T10:00:00', value: 10.2 }, // W03
    ]);
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    expect(result).toHaveLength(2);
    expect(result[0].label).toBe('W02 2025');
    expect(result[0].values).toEqual([10.0, 10.1]);
    expect(result[1].label).toBe('W03 2025');
    expect(result[1].values).toEqual([10.2]);
  });

  it('groups by 15-minute intervals', () => {
    const rows = makeTimeRows([
      { time: '2025-01-15T14:03:00', value: 10.0 },
      { time: '2025-01-15T14:12:00', value: 10.1 },
      { time: '2025-01-15T14:18:00', value: 10.2 },
      { time: '2025-01-15T14:29:00', value: 10.3 },
    ]);
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    expect(result[0].label).toBe('Jan 15 14:00');
    expect(result[0].values).toEqual([10.0, 10.1]);
    expect(result[1].label).toBe('Jan 15 14:15');
    expect(result[1].values).toEqual([10.2, 10.3]);
  });

  it('groups by 5-minute intervals', () => {
    const rows = makeTimeRows([
      { time: '2025-01-15T14:01:00', value: 10.0 },
      { time: '2025-01-15T14:06:00', value: 10.1 },
      { time: '2025-01-15T14:11:00', value: 10.2 },
    ]);
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    expect(result).toHaveLength(3);
    expect(result[0].label).toBe('Jan 15 14:00');
    expect(result[1].label).toBe('Jan 15 14:05');
    expect(result[2].label).toBe('Jan 15 14:10');
  });

  it('skips rows with missing time values', () => {
    const rows: DataRow[] = [
      { Weight: 10.0, Timestamp: '2025-01-15T14:00:00' },
      { Weight: 10.1, Timestamp: null },
      { Weight: 10.2, Timestamp: '' },
      { Weight: 10.3, Timestamp: '2025-01-15T14:30:00' },
    ];
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    expect(result).toHaveLength(1);
    expect(result[0].values).toEqual([10.0, 10.3]);
  });

  it('skips rows with unparseable time strings', () => {
    const rows: DataRow[] = [
      { Weight: 10.0, Timestamp: '2025-01-15T14:00:00' },
      { Weight: 10.1, Timestamp: 'not-a-date' },
      { Weight: 10.2, Timestamp: '2025-01-15T15:00:00' },
    ];
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    expect(result).toHaveLength(2);
    expect(result[0].values).toEqual([10.0]);
    expect(result[1].values).toEqual([10.2]);
  });

  it('preserves chronological order of buckets', () => {
    const rows = makeTimeRows([
      { time: '2025-01-15T10:00:00', value: 1 },
      { time: '2025-01-15T12:00:00', value: 2 },
      { time: '2025-01-15T11:00:00', value: 3 },
    ]);
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    // Map preserves insertion order — so data order, not chronological
    expect(result).toHaveLength(3);
    expect(result[0].label).toBe('Jan 15 10:00');
    expect(result[1].label).toBe('Jan 15 12:00');
    expect(result[2].label).toBe('Jan 15 11:00');
  });

  it('preserves original rows for factor lookup', () => {
    const rows: DataRow[] = [
      { Weight: 10.0, Timestamp: '2025-01-15T14:00:00', Operator: 'Alice' },
      { Weight: 10.1, Timestamp: '2025-01-15T14:30:00', Operator: 'Bob' },
    ];
    const config: SubgroupConfig = {
      method: 'time-interval',
      timeColumn: 'Timestamp',
    ...
    expect(result[0].rows).toHaveLength(2);
    expect(result[0].rows[0].Operator).toBe('Alice');
    expect(result[0].rows[1].Operator).toBe('Bob');
  });
});
...
describe('formatTimeBucket', () => {
  it('formats minute bucket with interval flooring', () => {
    const date = new Date(2025, 0, 15, 14, 23, 0); // Jan 15, 14:23
    expect(formatTimeBucket(date, 'minute', 15)).toBe('Jan 15 14:15');
    expect(formatTimeBucket(date, 'minute', 30)).toBe('Jan 15 14:00');
    expect(formatTimeBucket(date, 'minute', 5)).toBe('Jan 15 14:20');
  });

  it('formats hour bucket', () => {
    const date = new Date(2025, 0, 15, 14, 23, 0);
    expect(formatTimeBucket(date, 'hour')).toBe('Jan 15 14:00');
  });

  it('formats day bucket', () => {
    const date = new Date(2025, 0, 15, 14, 23, 0);
    expect(formatTimeBucket(date, 'day')).toBe('Jan 15');
  });

  it('formats week bucket with ISO week number', () => {
    // Jan 6 2025 is Monday of W02
    const date = new Date(2025, 0, 6, 10, 0, 0);
    expect(formatTimeBucket(date, 'week')).toBe('W02 2025');
  });
});
