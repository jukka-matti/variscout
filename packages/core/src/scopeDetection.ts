import type { ProcessStepCapabilityMember } from './processHub';

export type Scope = 'b0' | 'b1' | 'b2';

/**
 * Classify a per-step capability member by the cardinality of its nodeMappings.
 *
 ...
 */
export function detectScope(member: ProcessStepCapabilityMember): Scope {
  const mappings = member.metadata?.nodeMappings ?? [];
 ...
 *
 * Used by FrameView in surfaces that don't have a per-step capability member
 ...
 * nodeMappings exist downstream, `detectScope(member)` takes over.
 *
