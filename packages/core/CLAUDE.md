...
- **`ProblemStatementScope.projectId`** (PO-7 rename of `investigationId`) is the FK to the owning `ImprovementProject`; may carry `'general-unassigned'` for quick-analysis scopes with no active project. The **Control join key** (`ControlRecord/Review/Handoff.projectId`, also PO-7 renamed) may carry `${hub.id}:sustainment` — see the inline doc on `ControlRecord.projectId` in `src/control.ts`. Do not confuse with `ControlRecord.improvementProjectId` (the distinct optional direct project FK, NOT renamed).
