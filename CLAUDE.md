...
| docs/07-decisions/      | ADR-001 through ADR-044                                               |
...
| `apps/azure/src/`                             | context/DataContext.tsx, services/{localDb,cloudSync,storage}.ts, auth/, hooks/useEditor\*.ts, features/\*/, lib/appInsights.ts | Azure state, storage (IndexedDB + OneDrive sync), auth, Graph API, Zustand stores, App Insights |
