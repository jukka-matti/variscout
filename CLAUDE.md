...
Structured investigation for process improvement. Browser-based, customer-owned data, local-first. **V1 strategic direction (local-first reframe 2026-06-11, ADR-092 + ADR-093):** local-first **desktop** workspace for improvement specialists — practical Minitab replacement; 7-tab workflow nav (`Home · Project · Process · Explore · Analyze · Improve · Report`); collaboration = the **consultation loop** + Analysis Packs (live membership/roles/ACLs, the Azure persistence stack, and mobile surfaces are **deleted by decision — sweeps pending**, so that code still exists; do not build on it); three access channels: free in-session (no save/export, build-time gate) / individual €17+VAT/mo or €99+VAT/yr via Paddle (installable desktop PWA, BYOK CoScout) / company €120/mo via Marketplace (customer-tenant deployment, IT-governed CoScout). One app, three deployments — `apps/azure`+`apps/pwa` converge after the deletion sweeps. Canonical: [ADR-092](docs/07-decisions/adr-092-local-first-variscout-product-model.md) + [ADR-093](docs/07-decisions/adr-093-v1-simplification-cuts.md) + [consultation-loop spec](docs/superpowers/specs/2026-06-11-consultation-loop-design.md); the [V1 architecture spec](docs/superpowers/specs/2026-05-16-wedge-architecture-design.md) + [ADR-082](docs/07-decisions/adr-082-wedge-architecture.md) carry the surviving anatomy (Workspace model, 7-tab nav, stages). VariScout Process (enterprise) is named-future. Many docs pre-date 2026-06-11 — on conflict, trust ADR-092/093.
...
- **Tooling / docs / config** (`.claude/`, `scripts/`, `docs/`, `CLAUDE.md`, `package.json`): direct commit to main is fine. **Push doc commits to main BEFORE branching for code work** — otherwise they ride along on the feature branch and get bundled into the eventual merge under the PR's commit message (A2 / PR #215 lost 8 individual doc commits this way on 2026-05-26). **Product code** (`packages/*/src/`, `apps/*/src/`): branch → PR → `bash scripts/pr-ready-check.sh` green → subagent code review → **`gh pr merge --merge --delete-branch`** (preserve per-commit history; not `--squash`, which collapses per-category commits + review polish into one mainline commit — see `feedback_preserve_commit_history`). Pre-push hook warns (non-blocking) on direct-to-main product touches. Don't `gh pr merge --admin` unless emergency.
