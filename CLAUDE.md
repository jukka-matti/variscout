...
│   ├── index.md
│   ├── architecture.md
│   ├── architecture/    # offline-first, monorepo, shared-packages, data-flow, component-patterns, ai-architecture, ai-readiness-review, knowledge-model, ai-context-engineering, ai-data-flow
...
│   └── powerbi/
├── 09-tutorials/        # Step-by-step learning guides (Diataxis tutorial pattern)
│   └── index.md         # Tutorial index with planned topics
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Statistics/Cpk changes   | docs/03-features/analysis/capability.md, docs/03-features/analysis/stats-panel.md, packages/core/src/stats/index.ts                                                                                                                     |
...
| Staged Comparison        | packages/core/src/stats/staged.ts (calculateStagedComparison), packages/ui/src/components/StatsPanel/StagedComparisonCard.tsx                                                                                                           |
...
| AI integration           | docs/07-decisions/adr-019-ai-integration.md, docs/05-technical/architecture/ai-architecture.md, docs/05-technical/architecture/ai-data-flow.md, docs/05-technical/architecture/ai-readiness-review.md (Phases 1–3 delivered March 2026) |
| AI governance / AIX      | docs/05-technical/architecture/aix-design-system.md                                                                                                                                                                                     |
| Runtime config           | `apps/azure/src/lib/runtimeConfig.ts`, `apps/azure/server.js`                                                                                                                                                                           |
...
| File                                                                 | Purpose                                                                                                                                                                              |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `packages/core/src/stats/index.ts`                                   | Statistics engine (13 domain modules: basic, anova, boxplot, conformance, distributions, interaction, kde, modelReduction, multiRegression, nelson, probability, regression, staged) |
...
| `packages/hooks/src/useAnnotationMode.ts`                            | Chart annotation state (highlights, text notes, context menu); exported as `useAnnotations`                                                                                          |
...
| `packages/ui/src/components/DashboardBase/`                          | Shared dashboard building blocks (DashboardLayoutBase, FocusedViewOverlay, FocusedChartCard, DashboardChartCard, DashboardGrid)                                                      |
...
| `packages/core/src/parser/index.ts`                                  | CSV/Excel parsing, validation, keyword detection (shared)                                                                                                                            |
...
| `apps/pwa/src/components/layout/MobileMenu.tsx`                      | Mobile navigation hamburger menu                                                                                                                                                     |
...
| `apps/azure/src/components/data/DataPanel.tsx`                       | Azure data table panel (resizable, bi-directional sync)                                                                                                                              |
| `apps/azure/src/auth/easyAuth.ts`                                    | EasyAuth helper (AuthError, refreshToken, proactive token refresh)                                                                                                                   |
| `apps/azure/src/components/settings/SettingsPanel.tsx`               | Azure settings (theme, display, AI toggles)                                                                                                                                          |
...
| `apps/azure/src/lib/runtimeConfig.ts`                                | Runtime config loader (fetches /config for Marketplace deployments)                                                                                                                  |
| `apps/azure/src/services/indexService.ts`                            | Client-side trigger for indexing findings to Azure AI Search                                                                                                                         |
| `infra/functions/index-findings/index.js`                            | Azure Function: indexes findings to AI Search with tenant isolation                                                                                                                  |
| `infra/search-index-schema.json`                                     | Azure AI Search index definition for findings                                                                                                                                        |
...
| `packages/hooks/src/useAICoScout.ts`                                 | CoScout conversation state, streaming, abort control, onBeforeSend hook                                                                                                              |
| `packages/hooks/src/useKnowledgeSearch.ts`                           | Knowledge Base search wrapper (state, searchFn injection, enabled flag)                                                                                                              |
| `apps/azure/src/hooks/useEditorAI.ts`                                | AI orchestration hook: composes context, narration, CoScout, knowledge search for Editor                                                                                             |
| `packages/hooks/src/useHypotheses.ts`                                | Hypothesis tree CRUD, auto-validation (eta² thresholds), ideas                                                                                                                       |
| `packages/hooks/src/useAIContext.ts`                                 | AI context building hook (analysis state → prompt context)                                                                                                                           |
