...
- **Plan sizing + integration safety**: cap implementation slices at ~6–8 tasks per squash-merged PR — sequence multi-PR off one branch when scope is larger (slice 4 retro). Validate proposed integration targets with a focused Explore at plan time (does the consumer actually have the data in scope at the call site, not just typed?). For features spanning engine → primitive → app, write a §"Partial-integration policy" upfront so mid-flight reachability gaps trigger a pre-decided deferral, not an improvised fallback. See `feedback_slice_size_cap`, `feedback_plan_call_site_reachability`, `feedback_partial_integration_policy`.
