      ...
      try {
        ...
        for (const file of files) {
          try {
            ...
            // Strip markdown bold markers for status matching
            const statusMatch = content.match(/\*{0,2}Status:?\*{0,2}:?\s*(.+)/i);
            if (statusMatch) {
              const status = statusMatch[1].trim().toLowerCase();
              if (status.startsWith('accepted') || status.startsWith('implemented') ||
                  status.startsWith('superseded') || status.startsWith('deprecated')) {
                implemented++;
              }
            }
          } catch (e) {
...
function getTestStats() {
  ...
  function countTestFiles(dir, depth = 0) {
    if (depth > 5) return; // Limit recursion (5 for monorepo nesting)
  ...
  // Scan monorepo packages/apps for test files
  const monoDirs = ['packages', 'apps'];
  for (const mono of monoDirs) {
    const monoPath = path.join(process.cwd(), mono);
    if (fs.existsSync(monoPath)) {
      try {
        for (const pkg of fs.readdirSync(monoPath, { withFileTypes: true })) {
          if (pkg.isDirectory()) {
            const pkgSrc = path.join(monoPath, pkg.name, 'src');
            if (fs.existsSync(pkgSrc)) countTestFiles(pkgSrc);
          }
        }
      } catch (e) {}
    }
