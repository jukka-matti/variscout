---
title: Chart Design
audience: [analyst, developer]
category: reference
status: stable
---

# Chart Design

Styling, behavior, and implementation specs for all chart types.

- [Chart Styling Overview](overview.md)
- [Chart Colors](colors.md)
- [Responsive Charts](responsive.md)
- [Chart Hooks](hooks.md)
- [Shared Chart Components](shared-components.md)
- [Executive Mode](executive-mode.md)
- [Boxplot Charts](boxplot.md)
- [Capability Histograms](capability.md)
- [I-Chart](ichart.md)
- [Pareto Charts](pareto.md)
- [Performance Mode Charts](performance-mode.md)
- [Probability Plot](probability-plot.md)
