...
| onClick     | () => void              | undefined          | Click handler --- opens the Findings panel when provided. Adds `cursor-pointer`, `role="button"`, `tabIndex`, and keyboard support (Enter/Space). |
...
- `FindingsPanel` — Findings system that replaced the Investigation Mindmap
