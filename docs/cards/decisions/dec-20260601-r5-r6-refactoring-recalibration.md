...
R6 became the persistence boundary because `DocumentSnapshot` is now the shared unit for portable documents, PWA browser saves, `.vrs` export/import, and Azure local/cloud project persistence. A snapshot is hub-scoped: quick-analysis hubs may carry no formal Project, formalized hubs carry one live `ImprovementProject`, and the multi-hub `projectsById` mirror is not serialized.
