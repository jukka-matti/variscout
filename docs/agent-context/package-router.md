...
| **Chart components**                       | `packages/charts/CLAUDE.md`                                                    | `packages/core/CLAUDE.md`                                | ADR-002, ADR-005, ADR-051          | `.claude/invariants/charts.md`; `chartColors` only; `useChartTheme()`; pair `*-400` with `*-700` for labels                                                                                                          | —                      |
...
| **i18n strings / locale**                  | `packages/core/CLAUDE.md`                                                      | —                                                        | ADR-025                            | `.claude/invariants/i18n.md`; `registerLocaleLoaders()` before `preloadLocale()`; `formatStatistic()` not `.toFixed()`; `zh-Hans` → `zhHans.ts`                                                                      | `writing-tests`        |
...
| **PWA-specific (persistence / export)**    | `apps/pwa/CLAUDE.md`                                                           | `packages/stores/CLAUDE.md`                              | ADR-004, ADR-012, ADR-033, ADR-078 | Session-only by default; `.vrs` export/import only for durable document portability; no browser save identity; no cloud sync; no AI in free tier; `@source` directives required                                      | `store-state-glossary` |
...
| **CoScout prompts / AI**                   | `docs/01-vision/coscout-ax-design.md` (canonical entry point)                  | `packages/core/CLAUDE.md`                                | ADR-057, ADR-060, ADR-068, ADR-069 | `.claude/invariants/coscout-prompts.md`; `assembleCoScoutPrompt()` is entry point; tier 1 is cache-invariant; REF markers not raw data; never "root cause"                                                           | —                      |
...
| **Projects tab / project membership**      | `docs/superpowers/specs/2026-05-16-wedge-architecture-design.md`               | `packages/stores/CLAUDE.md`, `apps/azure/CLAUDE.md`      | ADR-082                            | Project stages: Charter→Approach→Sustainment (Handoff folded); **3 personas inside Project: Lead/Member/Sponsor**; per-project ACLs; Improve is a top-level verb tab (post-2026-05-16 amendment); €120/mo single SKU | `store-state-glossary` |
...
5. Check `.claude/invariants/` for machine-enforced constraints in the **Rules/Invariants** column. See `.claude/INVARIANTS.md` for the synthesizing index.
