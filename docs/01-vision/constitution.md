...
10. **7-tab workflow nav** — `Home · Project · Process · Explore · Analyze · Improve · Report`. The order matches the investigator's day. Improve is a top-level verb tab scoped to the Workspace's active Project (`useActiveIPContext(sessionHub)` is the internal accessor; `<NoActiveProjectGuidance>` empty-state pattern) — not a sub-step of another tab. (The earlier active-IP cascade framing was retired by the Workspace model, PR #358; analytical narrowing is the shared Analysis Scope lens.)
