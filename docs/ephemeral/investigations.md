...
## b0 ProcessStepsExpander mount loop — "Maximum update depth exceeded" [RESOLVED 2026-06-06]

Confirmed live in the FSJ-2 chrome walk (57 console errors): expanding **"Add process steps" at b0** mounts Canvas inside `ProcessStepsExpander` and triggers a setState→render loop; React aborts and the expander shows a broken SCOUT-ish placeholder ("Outcome not selected" despite a set outcome) instead of the canvas. **Pre-existing** (the expander composition shipped with the Canvas migration; the old canvas-keyboard e2e exercised the same path), but FSJ-2 makes b0 the primary paste landing — the bug is now one click from every fresh session. ~~Suspect: the `hydrateCanvasDocument` effect chain~~ **RESOLVED via PR 0 of the FSJ-3 split (systematic-debugging, stack-captured live 2026-06-06): the suspect was wrong** — the loop was `useCanvasHypothesisDrawing`'s reset effect depending on the whole `drawTool` result object (recreated by `useHypothesisDrawTool` every render) while `reset()` minted a fresh `{phase:'idle'}` each call, defeating React's setState bail-out. Shipped: bailing reset + memoized hook result + narrowed effect dep, with a composition regression test (the prior suites rendered the two hooks separately — the loop was structurally invisible; pre-fix it OOM'd the vitest worker). Chrome-verified 0 loop errors (was 427). The mount loop would have fired on ANY Canvas mount — b0-expander was simply the only live mount path on the PWA. The "SCOUT-ish placeholder" half of this entry is a SEPARATE pre-existing issue → next entry. The canvas-keyboard e2e skip remains (chip-rail-at-b1 reachability, unrelated to the loop).

## b0 expander opens at L1 SystemLevelView, not the L2 authoring surface [LOGGED 2026-06-06]

Surfaced by the PR-0 verification walk: with the loop fixed, expanding "Add process steps" at b0 renders `SystemLevelView` (L1 outcome digest — "Outcome not selected", n=0) instead of the L2 chip-rail authoring view the expander exists for. Two mechanisms: (a) the persisted `canvasViewportStore` viewport for a fresh hub carries `zoom: 0.2 → inferLevel → currentLevel 'l1'` (writer not yet traced — both defaults are 'l2'; something fits/zooms at b0 mount); (b) `SystemLevelView` derives its outcome from `map.ctsColumn` (empty at b0), not `projectStore.outcome` — by design for L1, wrong surface for b0. Candidate fix: the b0 expander mount should pin/fit the viewport to 'l2' (the authoring level — chip rail gates on `currentLevel === 'l2'`). Input to FSJ-4 (step-timestamps banner routes INTO step authoring) — fix before or with it.
...
## Finding note text truncates to one character + em-dash on capture cards [RESOLVED 2026-06-10]
...
**Resolved via PR #363 (2026-06-10):** same root as the "drops input after first keystroke" entry below — see its resolution note. Chrome-verified end-to-end: a 117-char note persists through capture → Findings drawer.

## Finding card shows dataset n, not condition n [LOGGED 2026-06-07] [RESOLVED 2026-06-10 via ER-0]
...
**Grounded 2026-06-10 (call-center walkthrough, reproduced as n=404 dialog → n=1600 card):** the dialog's `evidenceLabel` (condition-scoped, `packages/hooks/src/captureDraft.ts:80-122`) lives only on the ephemeral draft and is never persisted. On capture, `handleAddChartObservation` (`apps/pwa/src/App.tsx:680-686`; Azure mirror `useFindingsOrchestration.ts:208-214`) calls `buildFindingContext` with the draft's FILTERS but the dashboard's unfiltered ROWS (`filteredData`) — so the persisted `context.stats` (samples/mean/Cpk) ignore the condition the card's own chips claim. Fix shape: thread draft-computed condition stats (or selectedIndices) through `CaptureDraft` + `ChartObservationCaptureOptions` (`packages/ui/src/types/findingsCallbacks.ts:4`) into both app handlers. Part of the "numbers you can trust" cluster (see 2026-06-10 walkthrough entry).

**Resolved via ER-0 (2026-06-10):** both capture handlers (PWA `handleAddChartObservation` + Azure `useFindingsOrchestration`) now compute persisted `context.stats` over the condition's rows — `applyFilters(draft.filters, getState().rawData)` for same-tick derived columns; per-measure `measureSpecs` passed in both branches. Dialog n == card n.
...
**Still open; deferred to D4 (owner call 2026-06-13).** Grounded 2026-06-13: `ProblemConditionCard` (`packages/ui/src/components/AnalyzeWall/`) types `cpk: number` and always renders `Cpk {formatStatistic(cpk,…,2)}`. The fabricated 0 is **caller-side and app-divergent** — PWA `AnalyzeView.tsx:875` hardcodes BOTH `problemCpk={0}` and `eventsPerWeek={0}` (the metrics were never wired), while Azure `AnalyzeWorkspace.tsx:617` computes real `stats?.cpk ?? 0` (the `?? 0` is the only fabrication). The card-honesty change (undefined → "no specs set") is invisible without fixing the callers, and the PWA caller stub is exactly the mirror-divergence D4 reconciles. Fix it ONCE during D4 app convergence: card accepts `cpk: number | undefined` → "no specs set"; the unified AnalyzeView computes real scoped Cpk + events, passing `undefined` Cpk when the outcome has no usl/lsl. Not a demo blocker (demo gate retired).
...
Walk-verified before the 2026-06-10 clean pre-launch DB reset: after capture + hypothesis + evaluate, **every store in the PWA IndexedDB was at 0 rows**; a fresh tab showed a clean landing. The current reset database is `variscout-pwa-v1`, but the product issue is unchanged: the only durability is the beforeunload guard + the "Export this investigation" nag, yet the banner's first words are "**Finding saved.**" Known deferral (reload-durability → CS-0b; PWA-scope follow-up), but the walk makes it the single biggest first-session risk: 30 minutes of investigation lost on tab close unless the user noticed a yellow banner. Minimum demo-safe fix: reword the banner ("Captured — export to keep it"). Real fix: draft persistence to IDB + restore-on-return. Priority input for the design sessions.
...
## Capture-card Note field drops input after the first keystroke [RESOLVED 2026-06-10]
...
**Resolved via PR #363 (2026-06-10):** mechanism was focus-steal, not remount — `CaptureCard`'s combined mount effect (`cardRef.focus()` + document listeners keyed on `[onCancel]`) re-ran on every keystroke because all five parents pass inline closures, yanking focus out of the textarea after the first character. Fix: split mount-only focus from listener registration (`packages/ui/src/components/CaptureCard/CaptureCard.tsx`) + a harness regression test mirroring the Dashboard wiring (verified red pre-fix).
...
1. **[RESOLVED 2026-06-10] collapse the Dexie migration chain to a clean `version(1)`.** Executed in `codex/db-schema-version-alignment`: Azure and PWA now both declare one clean current Dexie schema version (`version(1)`) in fresh pre-launch reset databases (`VaRiScoutAzureV1`, `variscout-pwa-v1`) because VariScout has no customer IndexedDB upgrade contract. The reset also updates Dexie to the latest verified upstream release (`4.4.3`, GitHub latest as of 2026-06-10). Product boundaries remain intact: PWA stays export-only/session-first; Azure keeps its document/cache/sync overlay stores.
...
2. **Decompose the large files that survive the migration** (the holistic-eval's #11 render-decomposition, deferred until boundaries are stable). NOT `Editor.tsx`/`App.tsx` (mega-plan W6) or the portfolio `Dashboard.tsx` (shrinks with W4) or the i18n/`learnData` data files (large but not logic — leave). The real survivors: `CanvasWorkspace.tsx` (1696), `WallCanvas.tsx` (1554), `HypothesisCardWithPlans.tsx` (1493), `ProcessMap.tsx` (1223), `AnalyzeWorkspace.tsx` (1407), `ReportView.tsx` (1303), and `analyzeStore.ts` (1291, cross-surface store — holistic-eval #3).
   ...
   **Guardrail — judge by cohesion, not line count (don't mechanically shrink-to-a-number).** A unit is too big when it has **more than one reason to change**, not when it crosses N lines. Decompose only at **natural seams** (distinct responsibilities, reusable pieces) and **stop before over-fragmenting** — many tangled tiny files carry their own navigation tax and can be worse than one cohesive large file. Distinguish **accidental** complexity (the shells _mix concerns_ → decompose aggressively, via hooks + a view-switch — that's W6) from **inherent** complexity (canvas/Wall = pan/zoom + direct manipulation + multi-entity render → **organize** into layers/sub-components/hooks, don't force "small"). The shell shrink is a **side-effect of W2 (view-model) + W6 done right — not a standalone "make files smaller" project**; `analyzeStore` → Zustand slices. Per-unit test: _can you understand it without reading its internals, and change its internals without breaking consumers?_ If yes, leave it.
...
## Post-#358 doc propagation: remaining "active-IP cascade" narrative sweep [LOGGED 2026-06-09]

The Workspace-migration Apply-phase doc propagation (PR after #358) updated the named targets (`home.md`, `project-dashboard.md`, `ia-nav-model.md`) **plus** the agent-facing code-reference bugs caused by W3's `activeIPStore` deletion — fixed in the same pass: `store-state-glossary.md` (+ its SKILL.md + root `CLAUDE.md` store-count invariant: Annotation ×4→×3, View ×1→×2, total still 9 with `analysisScopeStore`), `stores-overview.md`, `onboarding-quick-start.md`, `project-reopen.md`, plus the retired `OtherProjectsList`/portfolio refs in `navigation.md` + `feature-parity.md`.

**Still owed (conceptual, not broken-code-ref — deferred to keep that PR scoped):** the **"active-IP cascade" user-facing terminology** still appears as the _live_ model in several vision/journey docs — `docs/01-vision/constitution.md:44`, `docs/02-journeys/index.md:20,26`, `docs/02-journeys/personas/index.md:18`, `docs/02-journeys/flows/azure-daily-use.md:217`, `docs/03-features/specifications.md:72`, `docs/03-features/workflows/index.md:55,59`, and `personas/lead.md`. These should reframe to "Workspace context + Analysis Scope" (active-IP retired by the Workspace model, PR #358). This sweep **overlaps the `2026-06-09-project-tab-v1-redesign` spec's planned propagation** (it already lists `ia-nav-model.md` + `lead.md` as targets) — fold them together. **Leave specs/ADRs/cards/archive untouched** — those are point-in-time records (ADR-078's `useActiveIPStore` row, the wedge/SDD specs) and correctly describe the model as built; ADR-082 already carries the supersession note.

**Also still owed (pre-existing interim banner, flagged in PR #357):** `docs/03-features/workflows/analysis-flow.md` is built on the retired 4-verb spine (content sound, structure stale) — faithful rewrite to the 5-verb `Frame → Explore → Analyze → Improve → Control` frame.

## Call-center user-testing walkthrough (2026-06-10) — grounded findings cluster [LOGGED 2026-06-10]

Owner-simulated improvement-professional session on the PWA (1,600-row × 25-col call-center CSV, planted Team×Queue interaction, Y=Handle_Time), every finding grounded in code by a 12-agent fan-out. Corroborates the FSJ-10 entries above (Cpk-0.00 card, no-link-finding-to-existing-cause, Wall ergonomics). Resolved on the spot via PR #363: capture-note focus-steal + the `.vrs` import button run-on. Grounded follow-ups, grouped into 4 candidate initiatives — **A. Wall papercuts** (ride the AW redesign), **B. Numbers you can trust**, **C. I-Chart at scale**, **D. First-session framing + chrome density**:

### A1. Wall factor glyphs render solid black — `fill-surface-primary` token doesn't exist [LOGGED 2026-06-10] [RESOLVED 2026-06-10 via ER-0]

`FactorGlyph`'s unfocused rect uses `fill-surface-primary stroke-edge` (`packages/ui/src/components/AnalyzeWall/FactorGlyph.tsx:72`), but the Tailwind v4 `@theme` block defines only bare `--color-surface` (`packages/ui/src/styles/theme.css:10`). Tailwind v4 **silently skips unknown candidates** → no fill rule → SVG default `fill: black`; the `fill-content` label is near-black-on-black. Affects BOTH apps (Azure's chrome-verify passed only because partial DOI opacity made black read as gray). ~104 occurrences across ~45 files use `bg-surface-primary` (grounded count; transparent-background failure mode on HTML, so unnoticed); the FSJ-10 "Map-Your-Data black-box sparklines" entry is plausibly the same family. Fix: `fill-surface` at the glyph + a `--color-surface-primary` alias in theme.css (repairs the latent users). Introduced CS-12 PR #294; not covered by AW-1..AW-9.

**Resolved via ER-0 (2026-06-10):** `fill-surface` applied at the `FactorGlyph` unfocused rect; `--color-surface-primary` added as a `@theme` alias of `--color-surface` in `packages/ui/src/styles/theme.css`, repairing all latent `bg-surface-primary`/`fill-surface-primary` consumers.

### A2. Analyze "Model" toggle is a visual no-op on any populated Wall (AW-1 × AW-2 regression) [LOGGED 2026-06-10] [RESOLVED 2026-06-11 via ER-3]

The toggle (`WallCanvas.tsx:1367-1375`) mounts `ModelBuilderBand` at SVG y≈960-1260 (factor-band row), but AW-1's readable `populatedViewBox` (`computeReadableWallContentBBox`, `wallLayout.ts:365-405`) deliberately crops to scope+hubs+findings (bottom ≈ y 768) — aria-expanded flips, band mounts in DOM, zero pixels change. Violates AW-2's own acceptance ("every relocated chrome element still reachable"). Works on cold-start only because `coldStartViewBox` covers the band. The seam test asserts DOM mount, not geometry (`WallCanvas.modelBuilder.seam.test.tsx:63`) — add a populated-branch visibility assertion. Fix: screen-space HTML overlay or pan-to-band on toggle.

**[RESOLVED 2026-06-11 via ER-3]** The in-SVG `ModelBuilderBand` is deleted. The Model toggle now opens `ModelDrawerBase` — a screen-space HTML right drawer, never inside the SVG viewport group, so the populated-branch geometry crop is structurally impossible. A populated-branch geometry guard was added to `WallCanvas.modelBuilder.seam.test.tsx`: the model surface element must NOT be a descendant of the populated `<svg>`, and no `<foreignObject>` inside `<g data-wall-viewport>` may have a y-extent that falls outside the parsed `data-wall-content-bbox`. DOI feed rewired: the drawer's `onModelStats({kept, deltaR2})` prop replaces `onModelStatsChange`; the factorEdges mutation-guard test retargets the new `WallCanvas.modelStats` prop. Glyph contribution bars and domain-weighted DOI are now always-live (not open-band-only).

### A3. "Seed 3 from Factor Intelligence" is `factors.slice(0, 3)` — selection order masquerading as ranking [LOGGED 2026-06-10] [RESOLVED 2026-06-11 via ER-2]
...
Walkthrough symptom: analyzing factor=Team, the presets said "Match Mon mean / Match Tue spread", and "Match best fully" recommended **mean +16.1** (worse, for a smaller-is-better Y) with σ −9% in green. Three grounded causes: (a) `apps/pwa/src/components/WhatIfPage.tsx:35` binds `activeFactor={factors[0]}` (first MAPPED factor) — Azure's twin correctly uses `viewState?.boxplotFactor`; PWA has the analyzed factor available in `useAnalysisScopeStore.boxplotFactor` and doesn't use it (pre-pivot line, commit 94b6d16ee). (b) The page reads only legacy global `specs`, never `measureSpecs[outcome]` — anything set via SpecEditor is invisible to What-If; the orphaned `useSpecsForMeasure` hook (zero consumers) exists for exactly this. (c) With empty specs, `inferCharacteristicType({})` → 'nominal' and `findBestSubgroup`'s no-target fallback anchors on `categories[0].mean` — "best" degenerates to first-encounter order (`packages/core/src/variation/bestSubgroup.ts:37-41`). Also: no save-scenario / take-to-Improve affordance, and the surface never surfaces `computeCumulativeProjection` (the What-If-anchored contribution the investigation-model spec settled on). Bigger question for a design session: the toolbar What-If is a pre-pivot abstract-μ/σ surface — decide fold-into-Analyze (factor-anchored scenarios) vs retire.

**Resolved via ER-0 (2026-06-10):** PWA `WhatIfPage` now binds `analysisScopeStore.boxplotFactor` (with a new picker write-mirror so the selection round-trips); both apps resolve measure specs via the formerly-orphaned `useSpecsForMeasure` hook (now adopted); `findBestSubgroup` returns `undefined` when no direction is inferable (no more first-encounter fallback); mean-anchored presets are suppressed when direction is unknown (spread presets remain). **The fold-into-Analyze vs retire design question remains open** — the surface's pre-pivot abstract-μ/σ framing is unchanged and still needs a design session decision.

### B2. Boxplot category ordering is alphabetical — weekdays render Fri·Mon·Thu·Tue·Wed [LOGGED 2026-06-10] [RESOLVED 2026-06-10 via ER-0]

`sortBoxplotData`'s 'name' branch is `localeCompare` (`packages/core/src/stats/boxplot.ts:107`); `BoxplotSortBy` offers name|mean|spread only. No weekday/month-aware ordering exists anywhere — ironically `DAY_ABBR`/`MONTH_ABBR` in `packages/core/src/time.ts:63/45` are used to EXTRACT derived day-of-week values, then the system sorts its own derivation alphabetically. Instant credibility hit for any Minitab/JMP user. Fix home: natural-order detection in the 'name' branch (all-weekday/all-month key sets) + export the ABBR tables; fixes both ui and hooks callers at once.

**Resolved via ER-0 (2026-06-10):** vocabulary-gated natural comparator added to `packages/core/src/time.ts` — Mon-first weekday order (full names + abbreviations) and Jan-first month order (full + abbr); `DAY_ABBR`/`MONTH_ABBR` exported. Wired into `sortBoxplotData`'s name branch and `determineStageOrder`'s auto-detect branch.

### C1. LTTB decimation documented-as-shipped but dead — no I-Chart call site ever passed `chartWidth` [RESOLVED 2026-06-13 via ER-10 #382 — LTTB now live via `useIChartModel`; ADR-039 + charts/CLAUDE.md amended to match]

ADR-039:52 marks LTTB "Implemented … integrated in useIChartData"; `packages/charts/CLAUDE.md` states violation force-inclusion as a live invariant. Reality: the integration commit (a0021d314, 2026-03-21) added optional params that **no live call site passes** (`packages/ui/src/components/IChartWrapper/index.tsx:145`, `apps/azure/src/components/views/ReportView.tsx:351`) — the only live LTTB consumer is canvas sparklines. At n=1,600 the I-Chart renders ~3,200+ SVG nodes (per-point `<g>` + ViolationPoint + capture affordance), no canvas renderer exists in packages/charts, and run rules fire on transactional row order (false "process shift" alarms from date-ordered Monday clusters). Walkthrough benchmark ask (owner): **Minitab/JMP feel at thousands of points**. Fix tier 1: pass width + activate LTTB with a marker-aware threshold (chartWidth×2 is sized for line fidelity, not marker count) — but FIRST lift Nelson 2/3 run rules out of `packages/charts/src/IChart.tsx:173-179` to the full-data hooks layer or decimation will corrupt them. Tier 2: marker-size-by-n + violation-only markers + connecting-line emphasis. Tier 3: canvas renderer / x-brush→TimeLens. Amend ADR-039 + charts/CLAUDE.md whichever way it lands.

### D1. Explore chrome: 5 stacked strips ≈205px before the first data pixel [LOGGED 2026-06-10] [RESOLVED 2026-06-10 via ER-1]

Strips grounded: (1+2) PWA `AppHeader` flex-wraps logo+7-tabs+chips+tool-strip into two rows above `lg` (labels `hidden lg:inline` — below 1024px it merges back to one row, so the compact layout already exists); (3) the framing toolbar (`apps/pwa/src/App.tsx:1327-1377`) mounts on EVERY tab against CCJ "canvas chrome" intent (Process-tab-only) and renders mostly empty; (4) `WorkspaceProjectScopeRibbon` duplicates the header's workspace+scope chips; (5) the stats strip duplicates numbers the I-Chart annotates. Fix homes per strip in the grounding; the I-Chart plot band ends up ~100px tall at default layout (~12% of viewport for the primary evidence). Pair with C1 for the "charts deserve the room" upgrade.

**Resolved via ER-1 (2026-06-10):** chrome collapsed to exactly two persistent rows — compact AppHeader (one row, icon-only tools, Findings badge) + ProcessHealthBar context line (stats + lenses: Subgroup/Time/Stages/Export/Edit-framing). Scope ribbon (`WorkspaceProjectScopeRibbon`) deleted across 9 mounts; framing toolbar gated to Process tab only; layout-toggle retired → scroll-only with viewport-relative I-Chart height. Stats strip absorbed into the context line's left cluster (`N rows · date range · x̄/σ/Cpk`). The "charts deserve the room" upgrade (C1/LTTB) is ER-10's scope.

### D2. Auto-X seed: undocumented `slice(0,3)` keyword bias + derived/CSV duplicate factors both selected [RESOLVED 2026-06-13 via ER-9 #378 — `factorSeedScore` replaces the keyword `slice(0,3)` bias (cap is prominence-only, never gates); `time.ts` adds the dedup/normalization gate]

`detectColumns` seeds selected X's = categorical columns keyword-sorted (`'day'/'team'/'site'` match; 'queue'/'channel'/'ivr' absent from `FACTOR_KEYWORDS`) then `slice(0, 3)` (`packages/core/src/parser/detection.ts:189-206`) — walkthrough effect: Weekday/Team/Site seeded, Queue (largest effect) left unselected. Independently, quiet time-extraction (FSJ §4.2) derives Date_Month + Date_DayOfWeek and **auto-selects them too**, with no value-equivalence check against existing columns — the dataset's own `Weekday` and the derived `Date_DayOfWeek` ride as duplicate selected factors into screening. Fix homes: dedup gate in `augmentWithTimeColumns` (`packages/core/src/time.ts`) + seed-cap/keyword rethink in detection.ts. The 3-cap is nowhere a recorded decision.

### D3. Findings without a hypothesis have NO Report destination; section checkmarks hardcoded green [RESOLVED 2026-06-13 via ER-11 #376 — explicit "findings not yet attached to a suspected cause" destination + content-derived section markers]

`selectIPReportScope` admits findings only via `hypothesis.findingIds`/`goal.mechanismGoals[].linkedFindingIds` (`packages/core/src/report/ipReport.ts:82-93`) — a captured-but-unattached Finding is invisible to the entire Report; "What we found + what we did" renders an empty body with **no empty-state fallback** (unlike its sibling sections) while every IP-overview section gets hardcoded `status:'done'` → green check (`apps/pwa/src/components/views/ReportView.tsx:201`; Azure mirror ~:538). PO-5 settled hypothesis destinations but never decided orphan-finding destinations; RPT-1 (merged #362) didn't touch this; CC-7's Report integration touches other sections — candidate rider there, or "B. numbers you can trust". Demo impact: the closing arc shows capture → Report says nothing, marked done.

### D4. Wall task assignment: type system + store support assignees, no UI captures one (PO mental-model correction) [LOGGED 2026-06-10]

Owner belief was "assignment exists, Azure-only" — wrong in both directions. The Wall "+ Add Task" form is text-only in BOTH apps (`HypothesisCardWithPlans.tsx:1152-1193`; its JSDoc claims an `assignee?` param the signature doesn't have, :194-197); both app handlers drop the existing `addHypothesisAction(hubId, text, assignee?, dueDate?)` assignee param; the card already renders `action.assignee.displayName` when present. Meanwhile the Measurement-Plan **owner** (member dropdown, sponsors excluded) works in BOTH apps including PWA — documented intent ("plan owner is the de-facto assignee", unified-canvas spec :72). The Azure-only artifact behind the belief (`FindingsPanel.tsx` `renderActionAssigneePicker`) is free-text (Graph PeoplePicker removed per ADR-059) and currently has zero mount sites. Smallest real gap: add a member select to the Wall task form + thread the param in the two handlers. Belongs to the AW redesign collaboration layer (named in the spec, not yet in AW-1..AW-9).

### Open question: Control as its own main tab post-CC-DOC? [LOGGED 2026-06-10]

Owner question during the walkthrough. Grounded inputs assembled (12-agent fan-out, full arguments in session record): Control today = 3rd Project stage, deliberately tab-less (D6 + ia-nav-model.md:126, re-affirmed by the 2026-06-10 control-closure spec §6:143 which mounts the new ControlVerificationBand inside the Project tab). FOR a tab: verb-frame symmetry (Improve's promotion is direct precedent, decision-log:124); the reframe makes Control a recurring chart-heavy analytical surface; the PWA already renders Control as a navless full-screen 8th view (`App.tsx:870-872`). AGAINST: dead chrome for most of a Workspace's life (Control activates late; meaningless for informal quick-analyses — the free-PWA majority); closure cohesion (wedge §3.2 "single end-of-project decision moment"); the reframe made Control chrome LIGHTER (review-board cut); workflow-order tension with Report's terminal slot; mid-flight churn on Codex CC-4/CC-6/CC-7. **Sequence: evaluate post-CC-DOC against the shipped band; if promoted it's a nav-model amendment (supersede D6, amend ADR-082 + ia-nav-model + wedge §3.1/§3.2 + decision-log).**

### Explore holistic sweep addendum (2026-06-10) — second pass, all controls exercised [LOGGED 2026-06-10]

Completes the walkthrough cluster above with a full-control sweep of Explore (specs set USL=600/Tgt=480, stages=Shift, all toggles driven). New findings, same A-D grouping: (1) **[RESOLVED 2026-06-10 via ER-0]** **Violin-plot checkbox hard-crashes BoxplotBase** — "Element type is invalid… check the render method of `BoxplotBase`" caught by the error boundary; a first-class display option kills the chart (→ B). Real cause: Vite 8/Rolldown CJS interop failure on `@visx/stats/lib` deep imports — resolved by switching to named root imports from `@visx/stats` + an architecture guard preventing deep-import regression. (2) **"Cpk stability" toggle is a silent no-op** even with specs + stages set — second dead-looking control after "Model"; no tooltip/disabled state explains what would enable it (→ B; check `resolveMode` capability preconditions). (3) **[RESOLVED 2026-06-10 via ER-1]** **Maximized I-Chart renders blank ~3-5s** (no skeleton) before painting — fixed by ChartSkeleton + one-rAF mount gate in DashboardChartCard/FocusedChartCard + worker-backed `useAnalysisStats` at 4 call sites; existing `isComputing` overlays now genuinely paint. Render DURATION (LTTB) is ER-10's scope. (4) **[RESOLVED 2026-06-10 via ER-0]** **Stage order not chronological** — "Auto order" renders Afternoon→Morning→Evening; same natural-ordering family as weekdays (B2). Fixed by the natural comparator in B2 above (day-parts: Morning→Afternoon→Evening). (5) **[RESOLVED 2026-06-10 via ER-0]** **Distribution histogram ignores set specs** — no USL/Target lines on the capability-relevant chart; "Mean" axis label collides with a tick label. Note: the fix was spec RESOLUTION (the Dashboards now feed the histogram `measureSpecs[effectiveOutcome] ?? specs`) not rendering; the Mean-label collision was addressed with a nudge offset. (6) **[RESOLVED 2026-06-10 via ER-0]** **Stats panel shows no Cp/Cpk with specs set** (Mean/Median/StdDev/n only) — the first place a practitioner looks. Fixed: `useAnalysisStats` and the Stats panel's `hasSpecs` gate now resolve `measureSpecs[outcome] ?? specs` inline per active measure (the `useSpecsForMeasure` hook itself was adopted in the two WhatIfPages). (7) **[RESOLVED 2026-06-11 via ER-4]** **I-Chart brush-selects points but offers no capture affordance on mouse-up** (boxplot ⊕ is the only chart-capture path; FSJ-7 brush-capture intent unreachable here). Grounding correction at build time: the PWA in fact HAD a brush→CaptureCard path (auto-opening capture card on mouse-up) while Azure had none — the real state was asymmetry, not total absence. Both paths are superseded by the **condition pill** (one pattern, both apps): a brush mints a pill with the honest split (n in-band, in-vs-out means) offering ✚ Capture (the finding stamps its condition + `scopeId`) and "view as condition →" (the scope bar + D6 tiers). The auto-CaptureCard and the desktop afterglow toast retired; the click-semantics change (chart click = Esc-clearable transient highlight; commit only via the pill) is the D6 commit-is-explicit reading; Azure's `ScopeChrome` mount deleted (the scope bar is the single condition chrome on Explore). The master plan's named range crack closed: a brushed y-band travels as `between`/`gte` ConditionLeaves through `syncScopeFromCondition` → `ProblemStatementScope` (predicateSetKey-idempotent) and "Take it to Analyze →" carries it; the Analyze re-anchor loops now translate `in` + range leaves, not just `eq`. (8) **The analytical depth exists but is buried**: maximize → carousel → boxplot view contains factor tabs + per-category stats table + one-way ANOVA + η² with plain-language "% of variation explained" (Team 9.3% vs Weekday 1.2% on the walkthrough dataset) — invisible from the default surface, behind two unlabeled affordances (⤢ then ‹›). **[RESOLVED-IN-PART 2026-06-11 via ER-2]** The ω²-adjusted η² ranking now surfaces as the factor strip on the **default** Explore surface (beneath the I-Chart hero), so guidance is no longer buried. Full ANOVA depth (SE, model fit, interaction plots) lands with ER-3's model drawer. **[FULLY RESOLVED 2026-06-11 via ER-3]** The drawer is the full-depth surface: model summary (S / R² / R²adj / n), reference-coded equation, full coefficient table with SE/t/p (including intercept row), Type III ANOVA table, best-subsets ladder, and predict widget — all accessible via the strip's ANOVA link (Explore) or the Wall's Model toggle (Analyze). The depth is no longer buried behind two unlabeled affordances. (9) **[RESOLVED 2026-06-10 via ER-1]** **Grid/Scroll layout toggle** (unlabeled icons) — toggle retired; scroll-only is now the default with viewport-relative I-Chart height (`h-[calc(100dvh-240px)] min-h-[500px]`). `DisplayOptions.dashboardLayout` deleted; `DashboardGrid` grid branch removed. (10) Spec-editor inference (USL-only → smaller-is-better) is invisible — no explicit direction choice, no confirmation of the inferred characteristicType. Designer-lens synthesis in the session record: the IA inverts the importance hierarchy (chrome at top, η² guidance deepest); fix direction = one chrome row + I-Chart hero + η²-led Variation Sources caption.

## The Condition as the unifying primitive — product-identity thesis from the Explore redesign [LOGGED 2026-06-10]

Surfaced at the end of the 2026-06-10 Explore-redesign design session (owner call: "needs to be thought through carefully in a holistic way — could change the nature of the project and usage"). The observation: every surface designed or grounded today converges on one noun. A **Finding is a condition + evidence** (settled, connective-surface spec). A **drill/scope is a condition applied as a lens** (`ProblemStatementScope`, ADR-085's "where"). An **inflection-bin segment is a condition discovered from distribution shape** (and must NOT re-enter the X ranking as a factor — circularity rule, see Explore-sweep addendum). An **interaction cell (Gamma×Billing) is a condition discovered from the model**. A **What-If scenario is a condition + counterfactual**. An **improvement action targets a condition**; **Control re-checks whether the condition still runs hot** (the widening-ladder re-checks are literally condition re-evaluation). A **signal (run-rule cluster) is a temporal condition**. CoScout's REF markers are condition references. The verbs of the 5-verb spine all act on conditions; the loop is a **tree of conditions** (problem statement at the root → drills → findings → causes → actions → control), and the Report is the narrated path through that tree. Code reality: `Finding.context.activeFilters` ({column: levels} conjunctions) is already the de-facto condition encoding; `ProblemStatementScope`, `ScopeFilter`, bin columns, and stage splits are condition-shaped siblings that don't share a type. **Thesis: name the primitive internally (one `Condition` shape: predicate + provenance: chart-capture / model-cell / bin / signal / manual), keep user vocabulary unchanged** (users keep seeing Finding / scope / segment — the system knows they're one species). What it buys: one capture flow everywhere; one engine question ("rank factors by association with a target — continuous measure OR condition membership") powering both strip variants; lineage for free (every condition knows its parent); Control = condition re-evaluation; cross-tab interaction grammar (hover/highlight/commit) acting on one noun. **Dangers:** ontology astronautics (this must stay invisible vocabulary), and mid-flight entity surgery (decision-log already queues "entity surgery = its own future brainstorm" — this thesis is that brainstorm's agenda, NOT a rider on the Explore-redesign spec). Sequencing: the Explore spec names the condition model conceptually (zero code cost — activeFilters already encodes it); unification is its own post-demo initiative. Product-identity stake: sharpens the wedge pitch from "charts + stats for improvement" to "a system of record for conditions and what you learned about them" — Minitab gives you analyses; VariScout keeps the investigation.

**Grounding correction (2026-06-10, same session):** the primitive is not merely "de-facto in activeFilters" — it formally exists THREE times, with bridges already written: (1) `Finding.context.activeFilters` ({column: levels} map, capture-time); (2) `ProblemStatementScope.predicates: ConditionLeaf[]` (`packages/core/src/findings/types.ts:822` — the entity already carries `outcome` + the flat-AND WHERE + `hypothesisIds` + `gateNode` contribution tree + a waiting `whatIfProjection?` field); (3) `Hypothesis.condition: HypothesisCondition` (`findings/hypothesisCondition.ts` — a FULL predicate-tree grammar: leaf ops eq/neq/lt/lte/gt/gte/between/in + and/or/not branches, designed to make hypotheses disconfirmable, auto-derived via `deriveConditionFromFindingSource` incl. probability-plot `between` ranges via `anchorYMax`). The **Issue** is NOT a condition — `AnalysisBrief.issueStatement` (types.ts:862, Stage 5 modal) is the prose intent at the root; the problem statement = issue (prose WHY) + scope (formal WHERE). So the corrected tree: Issue → ProblemStatementScope → Findings → Hypotheses → actions → Control. The surgery = CONVERGING three sibling encodings + wiring PSS (PR-CS-0 found drill→PSS had zero live callers — the Explore-redesign scope-drill loop is precisely the wiring PSS has been waiting for, and the what-if hover's permanent home is the existing `whatIfProjection` field). Owner methodology note that validates the membership reframe: the analyst's own practice is to brush the probability plot to identify the distinct underlying distributions (mixture components / process regimes), then attribute what drives membership — i.e., brush mints the SYMPTOM condition (a `between` leaf on Y), the membership strip ranks the X-side candidates, and the winner becomes `Hypothesis.condition` (the CAUSE-side condition) — two conditions linked by a disconfirmable claim. The entities anticipated exactly this; the redesign gives it a surface. **[DELIVERED 2026-06-11 via ER-5a]** The chain is live: the brush mints the condition (ER-4), the membership strip (bias-corrected Cramér's V, full-population per D7) ranks what distinguishes the rows with per-level lift, ⊕ composes compound conditions, and inflection segments commit as conditions (Azure binding editor).

### Pareto is question-gated, not mode-gated — defect mode decomposes into data-shape dispatch + a freed count view [LOGGED 2026-06-10] [DELIVERED 2026-06-13 via ER-5b]

Owner question at the end of the Explore-redesign session: was Pareto-only-in-defect-mode the right call, given ADR-089 retires mode/lens as user axes? Verdict: "defect mode" conflates two different things. (1) **Data-shape dispatch** — count/event-log Y needs different statistics (computeDefectRates BEFORE stats; never run the continuous engine on raw event logs) — this is engine-internal, stays, and should become _automatic detection_ of a count-shaped Y rather than a user-chosen mode (consistent with `resolveMode()` already being internal dispatch per ADR-047). (2) **Pareto availability** — a _question_ ("which categories dominate the counts?") that is valid on ANY data once a condition exists: the v4 mockup's membership view generated exactly this on continuous Handle_Time (count of slow-band calls per Team/Queue = a Pareto born from a condition, paired with the lift view so volume and signal stay distinguishable). Unified rule for the spec: **Pareto = the count-view of categories against a target, where the target is either a count-shaped Y (auto-detected → Pareto takes a primary slot + rate stats + the strip ranks by level-native defect-rate share per ADR-088) or condition membership (the count⇄lift toggle in the composition slot)**. No mode switcher in the redesigned chrome; `AnalysisMode` persists as internal strategy state pending the entity-surgery brainstorm. Caution preserved: no-modes ≠ one-stats-path — the transform-before-stats boundary is untouchable.

### The Frame/Process → Explore contract — five decisions the Explore spec must state [RESOLVED 2026-06-13 via ER-9 #378 (seed-not-gate + vocabulary symmetry) + ER-8 #380 (track-outcome); new `process-to-explore.md` journey flow]

Owner question closing the Explore-redesign session: is the Process(b0)→Explore flow seamless and good logic? Audit: the shipped mechanics carry well (paste→b0; Y/X selections, per-measure specs, run order, derived date columns all flow; "See the data →" lands on live charts) — but five seams need explicit spec decisions. (1) **Candidates seed, never gate.** Today the strip-equivalent surfaces only see SELECTED X's, and the auto-X seed is keyword-biased `slice(0,3)` (D2) — first-session Explore would rank 3 arbitrary factors and miss Queue. Decision: the η² strip screens ALL candidate factors; framing selection grants _prominence_ (featured/pinned), unselected candidates collapse under "also screened (+N)" — the analyst's framing directs attention, it must not blind the screen. This also makes the b0 seeding bug harmless rather than load-bearing. (2) **Vocabulary symmetry is the connective story:** Process asks "what MIGHT be affecting it?" (hypothesis-shaped selection); Explore's strip answers "what DOES explain it?" — same factors, question→answer. Name this in both surfaces' copy; it is the tab transition's meaning. (3) **Division of ownership for the return path:** Process owns data identity + process semantics (steps, run order, goal, re-ingest, renames); Explore owns analysis attention (active Y, factor prominence, conditions; specs shared-editable from both). With the Y-switcher + strip + spec entry in Explore, "Edit framing" trips back only for data/process changes — state that, so the framing toolbar's app-wide mount (D1) can die without losing a path. (4) **Goal/target/spec unification:** three target-ish concepts exist — the Process-tab "Set a process goal", `AnalysisBrief.target {metric,direction,value}`, and per-measure specs (USL/LSL/Tgt/CpkTarget). One story needed: spec = the measure's engineering limits; goal/brief-target = the project's improvement intent (lives with Issue); direction-of-goodness derives from spec, never from prose. (5) **Steps→stages mapping:** process steps (optional at framing, ADR-087 three step homes) should surface in Explore as the Stages lens automatically when steps carry a column mapping; for step-attributed X's, the strip's factor chips gain a step badge. Related fixes already logged: auto-X dedup/cap (D2), track-another-outcome routes to the dead wizard (Y-model section), Issue/opening-states redesign (FSJ-10 list).

## Seven more Tailwind tokens used but undeclared (allowlisted in the token guard) [LOGGED 2026-06-10]

The ER-0 architecture test `packages/ui/src/__tests__/architecture.tailwindTokens.test.ts` scans `@variscout/ui` className literals against the `@theme` block of `packages/ui/src/styles/theme.css`. Beyond the `--color-surface-primary` token fixed in A1, the test found 7 additional used-but-undeclared tokens, explicitly allowlisted in `KNOWN_MISSING_TOKEN_ALLOWLIST`: `surface-background`, `surface-tooltip`, `surface-hover`, `content-subtle`, `content-tertiary`, `edge-hover`, `edge-strong`. None has a raw CSS variable to alias — each needs a per-site rename to a real declared token, or a new variable added to theme.css (a design call, not a mechanical rename). Failure mode is the same silent-skip family as A1 and the FSJ-10 "Map-Your-Data black-box sparklines": Tailwind v4 silently emits nothing for an unknown candidate, leaving SVG fills black and HTML backgrounds transparent. The allowlist is the durable home for known debt — remove entries only when the rename or declaration lands.

## Spec-editor seeding still reads raw/global specs — write-surface family [LOGGED 2026-06-10]

Read-side per-measure spec resolution shipped in ER-0 (B1 + addendum items 5/6 above), but the spec EDITOR seeding is inconsistent and the same trust family: (a) Azure `Dashboard.tsx:1448` seeds the `SpecEditor` with `measureSpecs[outcome] ?? {}` — blank form when no per-measure spec exists, even if global legacy `specs` carry values; PWA `Dashboard.tsx` uses `?? specs` (falls back to global). (b) PWA `ProcessIntelligencePanel.tsx:155` passes raw legacy `specs` as the `SpecEditor`'s prop — any per-measure spec set via the form is invisible when reopening the editor. Save paths are correct in both (write goes to `setMeasureSpec`); the drift is open-form vs pre-filled UX only. Low stakes but same trust family — a spec that appears to need re-entry on every open erodes confidence in per-measure resolution.

Also in the same family, deliberately left untouched in ER-0 (lower stakes, may shift with the Explore redesign): `drillPath`/`cumulativeScope` persisted in `FindingContext` still reflect the dashboard drill state at capture time, not the draft condition — fine for now but drifts if those fields gain product meaning. Three optional natural-order adopters were left alphabetical: `factorChartData.ts:108`, `useMiniChartData.ts:50`, `EdgeMiniChart.tsx:180` — the last sorts before a `MAX_GROUPS` slice, so natural ordering changes which groups survive the cap and needs its own look before adopting.

## Dropdown close logic duplicated ~12× across packages/ui + two new copies in ProcessHealthBar [LOGGED 2026-06-10]

**Surfaced by:** ER-1 code review (commit 3870482bf) — the measure-chip menu and export menu in `ProcessHealthBar.tsx` each replicate the same open/close/Escape/outside-click pattern found ~10× elsewhere in the package (filter-chip dropdown, time-lens popover, stage-column selects, evidence-map context menus, and others).

**Pattern:** each site owns a `useState` open flag, a `useRef` for the container, and a `useEffect` that attaches `mousedown` + `keydown` (Escape) document listeners — ~15-20 LOC per site, essentially identical.

**Proposed fix:** extract a `useMenuClose(open: boolean, ref: RefObject<HTMLElement | null>, onClose: () => void)` hook into `packages/hooks/src/useMenuClose.ts`, export from the `@variscout/hooks` barrel, and sweep all ~12 call sites in `packages/ui`. Single source of truth for focus-trap/Escape behavior. Estimated reduction: ~150 LOC net across the package, and future sites get correct Escape handling by default rather than by copy-paste.

**Severity:** medium — functional today, but each site is a divergence risk (one site already guards `onEditFraming` inconsistently, caught in ER-1 review). Candidate for a standalone cleanup PR after ER-1 merges.

**Related token debt (same audit):** ~112 occurrences of raw Tailwind palette classes (`text-blue-400`, `bg-red-500`, etc.) exist in `packages/ui` className literals, sitting outside the token-guard's checked families (`bg-*`, `text-*`, `border-*`, `fill-*` are guarded only for semantic tokens declared in `theme.css`). The token guard (`architecture.tailwindTokens.test.ts`) does NOT yet scan palette-direct classes — they bypass the check silently. Candidate future guard widening: extend the test to flag `{color}-{shade}` pattern classes that don't have a matching `--color-{name}: {color}-{shade}` alias in `theme.css`, so V1 "no deep saturation, no dark mode" (constitution §V1 Wedge Principles) is machine-checkable.

## Dashboard remount on tab switch — recompute cost survives ER-1 [LOGGED 2026-06-11]

**Surfaced by:** ER-1 plan §Task 6 (out-of-scope note from the sub-plan).

**Summary:** both apps unmount the entire Dashboard on every tab switch — PWA via `AppViewSwitch` ternary (`apps/pwa/src/App.tsx`), Azure via `EditorViewSwitch` (`apps/azure/src/components/editor/EditorViewSwitch.tsx`). On return to the Explore tab, `useAnalysisStats` re-runs, chart mounts re-fire, and the full render pipeline re-executes from scratch. ER-1 fixed the **feedback** gap (ChartSkeleton + rAF gate + worker-backed stats mean the user sees a painted skeleton instead of a blank window while recompute runs) but did NOT fix the **recompute cost itself**. The keep-mounted option — React Activity component (`<Activity mode={visible ? 'visible' : 'hidden'}>`) or CSS-hide (`display:none` wrapper) — was deliberately not taken in ER-1; it is a structural change that deserves its own scoped look (interaction with Zustand subscriptions, scroll-position preservation, and potential stale-data concerns when re-mount is a natural cache-bust).

ER-10's LTTB revival addresses the **dominant render cost** (SVG node count). The recompute cost is a separate concern: worth revisiting once the LTTB + capability-lens work lands and we can benchmark tab-return latency with and without the keep-mounted option on real datasets.

**Note on Azure .vrs export:** ER-1 added a `.vrs` Export menu item to the context line's Export menu on PWA only. Azure's `exportToFile` has zero UI callers — the Export menu in Azure ships CSV-only (status quo, intentionally kept). Azure `.vrs` export remains UI-less until a future explicit decision.
...
## Spec §5 what-if mechanism pointer corrected [LOGGED 2026-06-11]

**Surfaced by:** ER-2 Task 1 grounding (plan §Grounding corrections adopted) and ER-2 Task 6 doc pass.

**Summary:** the Explore redesign spec (`docs/superpowers/specs/2026-06-10-explore-redesign-design.md`) §5 names `computeCumulativeProjection` as the hover-math home for the factor strip's what-if numbers. This pointer is incorrect: `computeCumulativeProjection` computes **complement-fixing** (what portion of the outcome would be accounted for if each factor's variation were eliminated successively) — a different quantity from the strip hover's "if all groups matched the best group's mean." The hover quantity is the new `computeMatchedBestProjection` (every group mean-shifted to the best group's mean, within-group spread preserved, direction-aware; returns `undefined` without a spec direction). **Amend the spec at next touch.** The permanent scope-level write-through is the existing `ProblemStatementScope.whatIfProjection` field, written by `recomputeScopeWhatIf` (now has its first live caller via ER-2 — the Task 1 "field only" comment in `types.ts:838` has been corrected). Per-chip hover numbers are ephemeral UI only and do NOT write to the scope field.

**Promotion path:** amend `docs/superpowers/specs/2026-06-10-explore-redesign-design.md` §5 on next spec touch (not its own PR — the spec is a living doc). Functional code is correct; this is a spec-prose drift entry only.
...
Three interactive behaviours that lived in `ModelBuilderBand` were **deliberately cut** when ER-3 replaced the band with the read-only `ModelDrawerBase`:

1. **Kept/candidate toggling** — the analyst could move a factor across the vital-few line (O(1) `lookupSubset` via the pre-enumerated subset table). The v1 drawer shows a static best-subsets ladder with the shown subset highlighted; toggling is not wired.
2. **Redundancy/VIF hint** — "removing X barely changed the model — redundant, not irrelevant" ambient honesty cue. The coefficient table in the drawer shows VIF values implicitly via SE (inflated SE under high multicollinearity), but the named redundancy hint is absent.
3. **Snap-back** — a "↩ Use suggested model" affordance to recover the engine's vital-few selection after a manual override. Not applicable while toggling is absent.
...
## `computeTypeIIISS` silently dropped the entire Type III decomposition on interaction-augmented models — anti-pattern logged [LOGGED 2026-06-11]

**Surfaced by:** ER-3 Task 1 `fitSubsetGLM` implementation + unconditional-assertion discipline in tests.

**Summary:** before ER-3, `computeTypeIIISS` (`packages/core/src/stats/typeIIISS.ts`) received the factor list from the base main-effects `BestSubsetResult` and compared reduced models against the interaction-augmented full model. Because the reduced models omitted interaction terms that the full model included, the partial-F comparisons were incompatible — the function silently returned an empty or incorrect decomposition for any interaction-augmented model. The bug was masked by two factors: (1) the all-categorical dispatch path (`computeBestSubsetsANOVA`) never returned `typeIIIResults` at all, so there was no existing consumer to catch it; (2) the pre-ER-3 seam tests for `computeTypeIIISS` used a main-effects-only fixture, so the failure mode on interaction-augmented inputs was never exercised.

**Fix (ER-3):** `computeTypeIIISS` is now hierarchy-aware — it uses the interaction-augmented factor list (including interaction terms) consistently across both the full model and the reduced models it compares, so the Type III SS decomposition is correct regardless of whether the winning model contains interactions.

**Anti-pattern to avoid:** the soft-skip test pattern — writing fixtures that only exercise the common/clean path and never exercise the edge case that would expose a silent-failure branch. Unconditional assertions ("this function must return a non-empty result for any non-degenerate input") surface these gaps early. The correct test discipline: for any function with a dispatch branch, write at least one fixture per branch that asserts the output shape, not just the happy-path value.

**Severity (pre-fix):** high for interaction-augmented models — the Type III table would either be empty or wrong, making the ANOVA section of the model drawer misleading. Fixed in ER-3; the regression guard in `fitSubsetGLM.test.ts` covers this path going forward.

## ER-4 adversarial review — accepted follow-ups (condition-loop edges) [LOGGED 2026-06-11]

The ER-4 final adversarial review returned READY with two IMPORTANT findings (fixed in-branch: the histogram mean line now derives from its own plotted bars under a condition; the header PersistentScopeChip × now also clears `projectStore.filterStack`) and four accepted follow-ups, logged here so they outlive the PR:

1. **ER-10 prerequisite — `DataPoints.tsx` membership re-keys on post-decimation index.** `conditionMemberIndices.has(i)` uses the rendered array position while `useIChartData` carries a per-point `isMember` flag (and force-includes members through LTTB). The two agree only pre-decimation; today no I-Chart call site passes `chartWidth`, so LTTB never runs and this is inert. **When ER-10 activates LTTB, `DataPoints` must consume `d.isMember` instead of re-keying on index** — otherwise the highlight tier silently mis-lights after decimation.
2. **Defect-mode condition partial application.** A group-click condition can be minted in defect mode, but `filterTierDataOverride` stays `effectiveData` there, so comparison charts don't filter while the scope bar shows and the I-Chart lights members. Index spaces stay aligned (no wrong-population read) — the application is partial, not dishonest. ER-4 scoped to the standard surface. **ER-5b landed (2026-06-13)** — auto-dispatch + defect-rate strip are now live, but `filterTierDataOverride` in defect mode is unchanged. Partial application remains. Revisit if a user-facing condition within defect dispatch becomes a product requirement.
3. **Azure brush-pill capture is thinner than the PWA path.** `handleBrushPillCapture` calls `onAddChartObservation('ichart', …)` without `anchorY`/`brushedRange`, so the finding lacks the y-band source anchor (the `scopeId` link works). Accepted Azure limitation in the Task 5 commit; candidate for the ER-7 findings-drawer slice where capture surfaces get reworked.
4. **Re-anchor writes categorical leaves twice** (scope-store `categoricalFilters` via `setCategoricalValues` AND the full predicate set via `setConditionLeaves`). Coherent — `clearScope()` runs first and Explore reads `conditionLeaves` under a condition — but redundant; collapse when the condition-entity surgery converges the sibling encodings.
...
**Addendum (same day) — capture UX, settled point 8:** adding bones must be whiteboard-fast and **free** (label-only; no required category/tag/link — an "Uncategorized" inbox branch + drag-to-classify; the <7-children rule is a soft hint). Entry paths: (a) type–Enter–Tab mind-map keyboard flow (the backbone), (b) drag from the also-screened candidates band / unbound columns / ER-5a segments (arrive pre-mapped + glowing), (c) paste an indented list (indentation = hierarchy), (d) live voice→CoScout propose→facilitator accept (reuses the loop's distill-propose-accept pattern; transcript-first voice exists), (e) in-flow "add to fishbone" from charts (pin-as-finding convention). First entity question for the spec session: a knowledge bone (typed, no data) needs a lightweight home — minimal `CandidateFactor` node `{label, category, parent, factorRef?, C/N?}` vs stretching D16's `HypothesisGroup`; the free-capture rule favors the dedicated node, since bones must be creatable without asserting any mechanism claim.
...
**[ROOT CAUSE FOUND + FIX IN FLIGHT 2026-06-14 — PR #399]** Systematic debugging (browser repro + reading the installed visx source) confirmed: the deprecated `withParentSize` HOC (`@visx/responsive`, marked `@deprecated`) renders NO child until its ResizeObserver delivers a measurement, gated through a `lodash.debounce(300ms,{leading:true})`, with no `initialWidth`/`initialHeight` fallback. On remount (AppViewSwitch is a ternary chain — every tab switch unmounts+remounts the lazy view) the observer can latch a transient 0×0 contentRect → chart renders at width 0 = blank until reload. INTERMITTENT (a race; confirmed by non-deterministic repro) → not a data/logic bug. **20 chart components shared the identical latent bug** (15 in `packages/charts/src/` + 5 app-local PWA forks). Fix (PR #399): one shared resilient `withResponsiveSize` primitive (seeds initial dims so first paint is never blank; **ignores degenerate <1px measurements** so a transient 0 can never latch) replacing all `withParentSize` usages; unit-tested ignore-zero guard (fails against old behavior). Mark RESOLVED once #399 merges + the live remount-stability browser re-check passes.

## Chart-component proliferation + cross-location duplication — consolidation review [LOGGED 2026-06-14]

**Surfaced by:** the PR #399 `withParentSize` refactor, which had to touch ~20 near-identical chart wrappers and revealed forked duplicates.

Two distinct smells worth a dedicated review (is the surface as large as it looks, and can it be smarter?):

1. **Cross-location duplication (highest-value, likely dead-code):** 6 chart files exist in BOTH `packages/charts/src/` AND `apps/pwa/src/components/charts/` — `IChart`, `Boxplot`, `ParetoChart`, `CapabilityHistogram`, `ProbabilityPlot`, `ChartSourceBar`. The PWA's `Dashboard`/`FocusedChartView`/`EmbedFocusView` import the **app-local forks**, not the shared `@variscout/charts` versions (only 3 PWA files import `@variscout/charts`). These app-local copies are older standalone implementations that drifted from the canonical package charts. **Question:** are the forks intentionally divergent, or stale duplicates the PWA should drop in favor of `@variscout/charts` (deleting ~6 files + their drift risk)? This is the cleaner half — likely a straight de-dupe. (PR #399 migrated the forks' sizing too rather than leave a half-fixed race, but did NOT de-dupe — that's this item.)

2. **Variant proliferation within `packages/charts` (design question):** the 18 components include parallel families — base (`IChart`/`Boxplot`/`ParetoChart`) vs **`Performance*`** (`PerformanceIChart`/`PerformanceBoxplot`/`PerformancePareto`/`PerformanceCapability`, the multi-channel performance-mode variants) vs **`Capability*`** (`CapabilityBoxplot`/`CapabilityHistogram`/`CapabilityGapTrendChart`) + `CpkTrajectoryChart`/`ScatterFit`/`StepErrorPareto`/`ProbabilityPlot`/`EvidenceMap`. **Question:** could the `Performance*` / `Capability*` variants collapse into a `mode`/`variant` prop on the base charts (or shared sub-components) instead of separate top-level components? Note ADR-093 D3 deleted mobile + the convergence removed Azure-only surfaces — some `Performance*` consumers may already be dead (the e2e triage flagged `performance-mode` as a deleted surface), so step 0 is a reachability sweep: which of these 18 are still mounted anywhere post-convergence? Dead ones delete outright; live near-duplicates evaluate for prop-unification.

**Why now:** PR #399 just made all 20 share one sizing primitive — a natural moment to evaluate consolidation while the chart layer is fresh in context. **Sequencing:** do the reachability sweep first (cheap, may delete several outright), then the cross-location de-dupe (#1, mechanical), then evaluate variant-unification (#2, design-judgment, own brainstorm if non-trivial). Not blocking; graduates to a spec/ADR or decision-log when picked up.

**Severity:** low (debt/maintainability) — no user-facing breakage; ~20 wrappers + 6 forks is real carrying cost (every cross-cutting change, like #399, multiplies across them).

## Consultation loop — V1 follow-ups (logged 2026-06-14, from CL-1…CL-6 adversarial reviews)

The consultation-loop arc (spec `2026-06-11-consultation-loop-design.md`, plan `2026-06-14-consultation-loop.md`) shipped deliberately scoped. Deferred items surfaced during per-slice review, none blocking:

1. **Hypothesis "Ask an expert" entry deferred (CL-5).** Finding-anchored ask shipped (FindingCard, board + list views); the HypothesisCard ask-entry was not threaded through the SVG `foreignObject`/WallCanvas this arc (`// TODO(CL-5)` in `ConsultationBuilder.tsx`). Follow-up: add an `onAskExpert` to HypothesisCard mirroring `onExplore`.
2. **Review-panel anchor-label resolves only hypotheses (CL-4/CL-5).** `ConsultationReviewPanel.resolveAnchorLabel` resolves `hypothesis` anchors to a name; `finding`/`scope` anchors fall back to the raw id. Since CL-5 seeds `finding` anchors, the panel shows a bare finding id. Follow-up: resolve finding/scope anchor names too.
3. **Consultation pack ships condition-only views (CL-5).** `resolveConsultationViews` + `buildConsultationPack` carry I-Chart/boxplot chart types end-to-end, but the AnalyzeView mount passes condition-only (`// TODO(CL-5): add chart views`) because the chart series isn't readily available at that mount. The loop is complete without it (condition + questions + template round-trip works); add chart views when the series is plumbed to the mount.
4. **Parser preserves stray markdown in answer bodies (CL-4).** A respondent who types a `###` line inside their answer has it preserved verbatim in the accepted text (the parser correctly anchors only on `[id:<uuid>]` headings, so it does not desync — it just keeps the stray text). Analyst can edit before Accept. Low.
5. **Review-panel anchor-label split-span locale ordering (CL-4).** The anchored-entity name renders in a separate span in fixed DOM order regardless of the catalog string; all 32 catalogs use the English placeholder today, but a future translation that moves `{label}` would render prefix/name in the wrong order. Revisit if/when these strings are truly localized.
6. **CoScout transcript distillation is dormant until provider wiring (CL-6).** `distillTranscriptToInsights` is provider-gated (returns `[]`, no network, when `config` is undefined — the reality in every channel today). The strict json_schema is now Azure-strict-valid (object root, full `required`, nullable `questionId`) but has never run against a live Responses API. When BYOK/tenant config is wired, integration-test the real round trip before relying on it — the `catch`→`[]` design means a schema/contract mismatch would fail silently.
