...
### Investigation-model design direction (Clusters A + B + C) — unified canvas · drill-to-condition · level×lens · Measurement-Plan-as-DCP
...
**Decision / next step:** all three clusters (A / B / C) settled; graduate to a combined **investigation-surface design spec** (`docs/superpowers/specs/`; add `[PROMOTED YYYY-MM-DD]` to this title when written) + **ADRs** (unified canvas; two-step-model reconciliation; level-native contribution; drop-`Question`; retire-mode/lens-axes + Values⇄Capability-as-the-one-view).
