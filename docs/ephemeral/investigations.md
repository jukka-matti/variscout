...
### Pareto is question-gated, not mode-gated — defect mode decomposes into data-shape dispatch + a freed count view [LOGGED 2026-06-10]
...
2. **Defect-mode condition partial application.** A group-click condition can be minted in defect mode, but `filterTierDataOverride` stays `effectiveData` there, so comparison charts don't filter while the scope bar shows and the I-Chart lights members. Index spaces stay aligned (no wrong-population read) — the application is partial, not dishonest. ER-4 scoped to the standard surface; revisit when ER-5b reshapes the defect/count-Y dispatch.
