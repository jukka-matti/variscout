...
### Investigation-model design direction (Clusters A + C) — unified canvas · drill-to-condition · Measurement-Plan-as-DCP

**Surfaced by:** Holistic design conversation 2026-05-29 (visual-companion brainstorm, opus). Settled the V1 investigation-model spine across **Cluster A** (PWA/Azure seam, #12 closure) + **Cluster C** (Findings/Hypotheses domain + canvas). **Cluster B (#11 Analyze-tab, #50 modes-vs-lenses, #51 Phase-2) is still open** — the three-level "level before mode" frame teed it up. Grounded against code (3 grounding workflows) + the methodology author's GB "Measure" decks (~144 pp).
...
- **Drill-to-specific-condition is the spine:** factor → level → combined condition (chips, **not** breadcrumbs); the **drill scope = the hypothesis condition = the Problem-Statement "where" = one `{factor=level}` predicate object** (`HypothesisCondition`). Bidirectional #51 falls out of this.
...
**Decision / next step:** direction settled; **awaiting Cluster B**, then graduate to a combined **investigation-surface design spec** (`docs/superpowers/specs/`; add `[PROMOTED YYYY-MM-DD]` to this title when written) + **ADRs** (unified canvas; two-step-model reconciliation; level-native contribution; drop-`Question`).
