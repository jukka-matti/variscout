...
- ~~Legacy functions preserved for backward compatibility during migration~~ **DONE 2026-05-30**: `legacy.ts` deleted; `BuildCoScoutToolsOptions` relocated to `tools/registry.ts`; `assembleCoScoutPrompt` is the sole prompt-assembly entry point (the coScout barrel still exposes `buildCoScoutMessageInput` and `getToolsForPhase` as distinct consumer entry points).
