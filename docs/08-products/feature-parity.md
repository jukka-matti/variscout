...
| **Investigation Wall — UI**          |    ✓    |      ✓       | Hypothesis-centric Wall projection (`packages/ui/src/components/InvestigationWall/WallCanvas.tsx`) — problem condition at top, hypotheses with mini-chart evidence, AND/OR/NOT composition into convergence branches, tributary chips live from ProcessMap, missing-evidence critique strip. Complements Evidence Map (factor-centric). Shared in both apps. See [Investigation Wall design](../superpowers/specs/2026-04-19-investigation-wall-design.md).                                                                            |
| **Investigation Wall — AI pipeline** |    -    |      ✓       | Background best-subsets detection + CoScout hint emission (`apps/azure/src/features/investigation/useWallBackgroundJobs.ts`). PWA's equivalent is a deliberate no-op stub — the Wall renders, but no proactive AI suggestions surface in the free tier.                                                                                                                                                                                                                                                                                |
...
| **Sustainment — UI**                 |    ✓    |      ✓       | Sustainment closure surface (`SustainmentPanel` / `SustainmentRecordEditor`) — Cpk delta, action completion, drift since closure, Mark complete / Reopen. Shared in both apps.                                                                                                                                                                                                                                                                                                                                                         |
| **Sustainment — persistence**        | Session |   Durable    | Azure persists Sustainment records via the R13 direct-write exception (IndexedDB + Cloud Blob); persistence is durable across sessions. PWA Sustainment is session-only — records do not round-trip through `.vrs` export in the current implementation (R13 exception applies in both apps; F5 promotion to HubAction-dispatched flow is deferred). See ADR-080 for the planned auto-fire pattern.                                                                                                                                    |
| **Report Workspace**                 |    ✓    |      ✓       | Report as a workspace tab. PWA: session-only report view + copy/PDF. Azure: + Share, Save As. Sponsor role on a Project reads everywhere + edits contributions (2-tier ACL with Member).                                                                                                                                                                                                                                                                                                                                               |
