...
**Previous architecture (ADR-016, superseded):** Team tier required 5 admin-consent Graph API scopes (`Files.ReadWrite.All`, `Channel.ReadBasic.All`, `ChannelMessage.Send`, `Sites.Read.All`, `People.Read`). These have been removed in ADR-059 (Web-First Architecture). Azure App collaboration now uses Azure Blob Storage with Azure RBAC instead of Graph API.

---

## Blob Storage Authentication (Azure App)

Azure App projects are stored in Azure Blob Storage in the customer's resource group. R6e storage access is server-enforced through same-origin App Service APIs:

```
Browser → App Service storage API
    → Validates Entra ID session (EasyAuth)
    → Checks the R6c document access model
    → Reads/writes Blob Storage with App Service managed identity
    → Returns only authorized document data
```

The App Service uses its managed identity (with `Storage Blob Data Contributor` RBAC role) for production Blob Storage operations. No storage credentials or broad project-data container SAS tokens are exposed to the browser.

### Security Controls

| Control                | Detail                                                                  |
| ---------------------- | ----------------------------------------------------------------------- |
| Server-enforced access | Same-origin APIs apply the R6c document access model before Blob I/O    |
| Entra ID gated         | Storage APIs require valid EasyAuth session                             |
| Managed identity       | Production Blob Storage access uses App Service managed identity        |
| No browser broad SAS   | Browser code must not receive a broad project-data container SAS        |
| Local dev/test keys    | `AZURE_STORAGE_CONNECTION_STRING` / Shared Key is local-dev/test-only   |
| Azure RBAC             | Customer IT manages resource access via standard Azure role assignments |

Source: `apps/azure/server.js` (same-origin storage API routes)
...
All Azure App users use `User.Read` + `People.Read` scopes. Both are user-consent, zero admin consent required.
