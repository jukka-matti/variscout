...
The Control path auto-fires per [ADR-080](07-decisions/adr-080-control-auto-fire-pattern.md); the Handoff path is deleted everywhere (folded into Control closure).
...
| **Improve stage** (inside Project detail) | [Improve tab](#improve-tab) (top-level verb tab)                           | 2026-05-16 amendment — Improve promoted from a Project stage to a top-level verb tab with active-IP cascade. Project lifecycle is now Charter → Approach → Control (3 stages). PDCA workbench available behind an "Advanced" toggle in the Improve tab. |
| **Handoff stage**                         | Folded into [Control](#)closure                                            | Single end-of-project decision moment ("did it work? + close"). Handoff actions captured in Control notes.                                                                                                                                              |
| **Handoff response path**                 | Control auto-fire (ADR-080)                                                | Canvas no longer exposes a Handoff path. Control auto-fires when Improve actions complete or cadence triggers fire.                                                                                                                                     |
