...
**V1 single-SKU architecture SHIPPED 2026-05-13 → 2026-05-17.** Canonical spec at [`docs/superpowers/specs/2026-05-16-wedge-architecture-design.md`](superpowers/specs/2026-05-16-wedge-architecture-design.md); architectural record at [`docs/07-decisions/adr-082-wedge-architecture.md`](07-decisions/adr-082-wedge-architecture.md). Splits VariScout into two products on a roadmap — V1 ships first (project tool for improvement specialists, single €120/mo SKU, project-membership ACLs, 7-tab nav per the 2026-05-16 Improve-tab amendment, Improve as a top-level verb tab with active-IP cascade); VariScout Process defers as named-future for process ownership / Hub portfolios / 4-persona model / auto data pipelines. Supersedes ADR-007 (in part) + ADR-033 (in part) + Coherence audit (Session A retires; Sessions B+C amend). Engineering wave PR-WV1-1..6 SHIPPED via PRs #182, #183, #185, #186, #187 + Improve-tab restoration. Phase A canonical-anchor doc cleanup landing as a single docs PR (this work). Phase C holistic ~500-doc audit scheduled post-customer-validation. Decision-log entry pinned 2026-05-16, amended 2026-05-17 with Phase A/C execution split + price amended €99 → €120.

## 2. In flight

**Phase A canonical-anchor doc cleanup (this PR, 2026-05-17).** Closing the V1 single-SKU spec §13 commitment: 7 anchor docs (OVERVIEW, USER-JOURNEYS, DATA-FLOW, positioning, business-bible, tier-philosophy → membership-philosophy, roadmap, llms.txt, root CLAUDE.md) rewritten to reflect V1 anatomy + €120/mo + project-membership ACLs + two-product roadmap. Phase C plan artifact created for the post-validation holistic ~500-doc audit. Sequenced as PR 1 → PR 1.5 (VariScout Process capture-spec) → PR 2-4 (Phase C AMEND / ARCHIVE / DELETE). See decision-log 2026-05-17 entry.

**~~Coherence audit + design~~ — partially superseded 2026-05-16** by the V1 single-SKU pivot. Coherence Session A's 4-persona model retires for V1 (migrates to VariScout Process); Sessions B + C refocus on single-persona V1. Spec [`docs/superpowers/specs/2026-05-14-variscout-coherence-design.md`](superpowers/specs/2026-05-14-variscout-coherence-design.md) marked partially superseded; pieces still in force (Session B vocabulary, Session C visual identity + a11y) live in the V1 architecture spec.
...
### Other workstreams (post-V1-launch)
...
| 13  | **Security hardening implementation** (concept doc landed 2026-05-06 at `19e2e5a4`)                                                                                                | L (multi-PR rollout)                | Paid-customer trust; auth + access control + SAS scope reduction  | Concept → spec brainstorm        |
...
| 15  | **Phase C holistic doc audit** (V1 spec §13)                                                                                                                                       | M (3 PRs: AMEND / ARCHIVE / DELETE) | Closes ~500-doc cleanup commitment; archive over delete           | V1 customer validation           |
| 16  | **VariScout Process — capture-spec + ADR** (PR 1.5 follow-up to this Phase A)                                                                                                      | S (1 doc PR, Opus)                  | Captures the named-future enterprise product before customers ask | None                             |

### VariScout Process — named-future (separate product, post-V1-validation)

When V1 reaches ~500 customer validation, VariScout Process opens design. Scope: enterprise platform for ongoing process ownership (multi-Hub portfolios, 4-persona routing — Process Owner / Project Lead / SME / Frontline, auto data pipelines, Process Measurement System as a separate surface, multi-source ingestion at Hub level). Different customer (process owners, OpEx VPs, plant managers), different unit economics (higher ARPU per tenant), different sales motion (enterprise vs Marketplace-first). Pricing TBD. Not announced in V1 marketing; mentioned only when customers ask. Captured pre-design in PR 1.5 capture-spec.
...
- Team-collaboration features inside IP / Sustainment / surfaces (signoff, audit trail, alerts, RACI, change notifications) — under V1 single-SKU, these become project-membership-gated (not tier-gated). Sponsor signoff at lifecycle gates is out-of-band at V1 per ADR-082 §4.1; audit trail / comments / RACI / notifications stay V2 named-future.
...
6. **Access-gate inside surfaces, not at entry CTAs** (`feedback_tier_gate_inside_surface`, repurposed under V1 single-SKU). Document authoring + structured workflows serve free-tier pedagogy + `.vrs` export; project-membership ACL checks happen inside the surface, never at the tab boundary.
