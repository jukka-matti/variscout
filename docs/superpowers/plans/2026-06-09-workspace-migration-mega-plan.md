...
- **⚠ Owner micro-decision before the prompt:** does Azure Home become a **flat Workspace list**, or **resume-last-Workspace + create-new** (no list)? The spec says "open or create Workspaces" but doesn't dictate list-vs-no-list.
- **Scope:** reshape Azure `pages/Dashboard.tsx` from the hub-dropdown + analysis-card portfolio (`:271-302`, header `:175-178`, per-hub filter `:112-122`, "New Hub" `:186`) into the chosen Workspace shape consuming `WorkspaceViewModel`; delete `OtherProjectsList.tsx` + its render/props in `ProjectDashboard.tsx:25-27,218-224`; trim `useProjectOverview.ts` to drop the `listProjects()` portfolio fan-out (keep `lastViewedAt`); rework `App.tsx` `mode="portfolio"` header + "Go to Portfolio" + auto-redirect (`App.tsx:242-250,274,291`). Fold the **Slice 2 vocabulary** sweep here (plural "Projects"→Workspace framing in `ProjectsTabView`, PWA `HomeScreen` copy).
- **Acceptance:** no `OtherProjectsList`/`onViewPortfolio`/"Portfolio" in non-test code; Home renders the chosen Workspace shape; browser-verify create + open + the first-session flow not regressed.
