...
**Follow-up (separable spec-lifecycle hygiene, not blocking):** flip the upstream `2026-05-29-investigation-surface` + `2026-05-31-factors-evaluation` specs `draft → delivered` (each needs a `delivered-by` + Delivered banner per the validator); the Apply-phase sensor will then confirm their `implements:` targets are applied.
