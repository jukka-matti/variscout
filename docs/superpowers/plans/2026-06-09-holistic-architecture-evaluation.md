...
The selected V1 model is `Workspace -> soft-formalized Project -> current Analysis Scope`.

- Workspace is the user-facing object: the place where an analyst brings data and investigates.
- Project is always present as the Workspace backing record, with soft formalization adding membership, role context, charter/status, actions, and formal report prominence.
...
1. **Workspace/Product Model Lock.** Use the Workspace architecture spec to define Workspace, soft-formalized Project, Analysis Scope, and internal `ProcessHub`.
2. **CoScout structural autonomy boundary and eval plan.** Define what CoScout may answer, propose, draft, route, and apply using Workspace + Project + Analysis Scope context.
