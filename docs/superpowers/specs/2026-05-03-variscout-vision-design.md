...
- **PWA = session-only by default with opt-in local persistence.** Users can explicitly opt in via a "Save to this browser" action (IndexedDB-backed Hub-of-one — single Hub, browser-tenant-only per ADR-059, Constitution P1 customer-owned data) AND/OR export their work as a `.vrs` file (file-based persistence; user owns the file). Both paths are always available; the user picks. Same Canvas UX as Azure once data is loaded. Multi-Hub portfolio + cloud sync + cadence-driven Evidence Sources + CoScout + team features remain Azure-tier exclusive. **Companies' factory data still flows to their own Azure tenant per ADR-059** — PWA persistence is for individual / training use (LSSGB students; demos; casual analysts), not company governance. Constitution P8 (no AI in free tier) preserved.
...
| Q8  | PWA-vs-Azure tier split (revised 2026-05-03)   | PWA = session-only by default with opt-in local persistence (IndexedDB Hub-of-one) AND always-available `.vrs` export/import. `.vrs` doubles as a **shareable training scenario** — trainers package datasets + Hub state for students to import. Azure adds cloud sync + multi-Hub + cadence-driven Evidence Sources + CoScout + team features. See decision-log "Q8 revised" entry. | §7 ("Tier split") |
