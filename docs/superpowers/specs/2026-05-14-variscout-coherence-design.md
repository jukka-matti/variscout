...
### Top-level navigation (Path A — 6 tabs, one job each)

```
[Home] [Process] [Analyze] [Investigation] [Improvement] [Report]
```

Six top-level tabs. Each passes the Linear test (describable in one sentence). The dual-purpose "Process tab with Map/Analyze toggle inside" pattern is retired in favor of honest separation.

| Tab               | One sentence                                                                     | Primary persona             |
| ----------------- | -------------------------------------------------------------------------------- | --------------------------- |
| **Home**          | Your personal queue — what's waiting for you right now                           | All (persona-adaptive)      |
...
| **Improvement**   | IPs lifecycle — from charter to sustainment to handoff                           | Project Lead, Process Owner |
...
**Improvement** = IPs lifecycle. Single home for IP cards + IP detail (Overview/Sections toggle). Other surfaces reference IPs via badge-links; never duplicate IP cards.
...
- **IP detail page details** — Overview / Sections toggle + team workspace pattern (Team roster + Activity feed) need detailed visual design.
