...
CS-P2…P5 content · the Home-launchpad design (consumes the §8 attention contract) · the Control closure model (initiative #12; consumes the re-homed region) · F3 storage normalization (named-future; this spec is a prerequisite simplification) · Evidence-Map post-Model-B fate (parked, `investigations.md`) · CoScout prompt redesign (context-slim only).
...
| F3 storage normalization       | the dissolved entity surface (fewer entities to normalize)                  | named-future                            |
...
## §9 · Data-growth posture

Evaluated 2026-06-04 (owner ask: "if we add more data, can we still handle it well?"):
...
- **Cross-document queryability: the deliberate trade.** Dropping the FK + indexes + stub read-APIs means "all findings across projects" would require loading every blob. V1 never runs that query (ADR-073 forbids roll-up across heterogeneous units; the portfolio reads projections). If a future needs it → F3, which inherits a _simpler_ entity surface.
...
**Dependencies**: A → {B1 → B2 → C} ∥ {D1 → D2} → E. CS-P2 starts any time post-spec. Per PR: grounding → sub-plan → subagent build → adversarial review → merge; one worktree per PR; delivery state lives in `gh pr list` + the master plan, not memory.
