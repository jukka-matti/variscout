...
| **Point at the probability plot** (point/region — amended 2026-06-06) | the value band — `Y between 4.2–5.1` (`FindingSource chart:'probability'` is shipped; condition derivation already yields the `between` leaf) | **mints a derived value-band factor on save** (a carve gesture, like brush) |
...
- **Probability plot (amended 2026-06-06, owner):** capture at this chart was planned from the start — the archived 2026-03-29 enhancement spec specified brush-region → factor/finding, and the domain plumbing shipped (`FindingSource chart:'probability'` + between-leaf derivation, Wall spec table). CCJ §3.5 **inflection binning is the SAME grammar already live here** — engine proposes cut guides (cyan), analyst decides, _"Create binned factor"_ ≡ this card's **Factor only** action. FSJ-7 converges them, not coexists: point-at-plot opens the capture card with the value-band condition pre-filled; the binning side panel's create-factor exit routes through the card's Factor-only path (one grammar, one card). **Brush stays I-Chart-only in V1** — probability-plot brush is the archived spec's revive trigger, not this build.
