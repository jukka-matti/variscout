...
- **The analytical tool, auto-suggested from each factor's data type:** categorical → **boxplot + 2-sample**; continuous → **scatter + regression**; a spread question → **capability (Cp/Cpk)**. The engine pre-picks; the analyst confirms. _(Delivered CS-9, 2026-06-03 — the spec §4.0 "sees the actual chart" step: the suggested chart now renders **inline in the triad row** — a `MiniScatterFit` (continuous) or `MiniBoxplot` (categorical) — **summoned onto the focused hypothesis, not always-on** (gated on `focusedWallEntityId`), so the analyst sees the actual chart before tapping Evaluate. Pure derivation `deriveScatterFitData`/`groupOutcomeByFactor` reuses the existing OLS engine — no new stats. **Cp/Cpk deferred** to a follow-up — there is no spread-question trigger in code (`suggestToolForFactor` never emits `capability`); the boxplot+2-sample and scatter+regression charts the engine already supports ship now. The explicit analyst-owned support/counts-against call stays **CS-10's** de-automation, not CS-9.)_
...
5. **UX/CoScout support for the disconfirmation loop — DESIGNED (2026-05-31): see §4.2** (the "Try to break it" checkbox on the evaluate; engine-graded; CoScout proposes-one-rival; staged Increment-2 UX vs CoScout-later). Residual calls for the plans: (a) the falsifying-**prediction field** — RESOLVED (2026-05-31, product owner): **optional but strongly prompted** (placeholder + hint); the engine verdict is the real backing regardless, so don't gate the checkbox on a written prediction; (b) the fused toggle **depends on §4's evaluate→typed-finding path** (net-new) — so "fix the `[]` gap" can't precede the evaluate; sequence the evaluate + the disconfirmation fusion as one slice (or give the gap a manual-form interim); (c) relocation finding-carry — show the supporting-carry explicitly on the verdict screen (not a silent sign-flip); (d) CoScout confound detection — compute deterministically + emit only the result (cheaper + engine-authoritative) vs projecting per-hub sign into `AIContext` every turn.
