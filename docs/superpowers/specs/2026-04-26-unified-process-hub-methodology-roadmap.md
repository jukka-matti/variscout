...
| Data-team / consultant enable | Data team, external consultant      | Fit owned source-system extracts to VariScout import contracts    |
...
- Define customer-owned snapshot/import contracts for data teams and
  consultants.
- Keep extraction and transformation outside VariScout.
- Reserve a logical future hub namespace such as `process-hubs/{hubId}/...` for
  snapshots, documents, and sustainment artifacts when a slice needs it.
...
1. A reader can explain the difference between Process Hub, the investigation
...
2. User journeys describe quick analyst work, process-owner cadence, multi-hub
   GB/BB work, PWA training, data-team enablement, and sustainment/control.
