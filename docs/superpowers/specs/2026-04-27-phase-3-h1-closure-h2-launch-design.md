...
8. **Collaborative real-time presence on team notes.** PR #1 saves notes through the existing Blob Storage round-trip and relies on the R6 document ETag conflict path rather than silent last-write-wins. Real-time collaboration is its own architecture problem.
