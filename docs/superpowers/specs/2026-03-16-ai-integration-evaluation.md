...
| 1.2 | Source attribution           | 2     | `promptTemplates.ts:394-421` — `formatKnowledgeContext()` includes `[From: {source}]` in system prompts. `CoScoutMessages.tsx:80-87` — knowledge result count shown during loading. | Source citations are injected into prompts but never surfaced in assistant responses. User sees "Searching knowledge base..." during load but no attribution in the actual response text. |
