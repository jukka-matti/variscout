...
graph TD
    ...
    TH -->|navigate_to: showDashboard, showAnalysis, showInvestigation, showImprovement, setCoScoutOpen, openReport, setPendingChartFocus| PS
...
| `teamToolHandlers.ts`              | `panelsStore`        | `showDashboard()`, `showAnalysis()`, `showInvestigation()`, `showImprovement()`, `setCoScoutOpen()`, `openReport()`, `setPendingChartFocus()` (ADR-055)                                                                                                             | `navigate_to` tool handler navigates to workspaces                        |
...
| ------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `Editor.tsx`              | `panelsStore`        | `activeView`, `isFindingsOpen`, `isCoScoutOpen`, `isWhatIfOpen`, `isReportOpen`, `isStatsSidebarOpen`, `pendingChartFocus` (ADR-055: `isImprovementOpen` removed) | Workspace routing, panel visibility                   |
