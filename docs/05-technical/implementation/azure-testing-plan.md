...
| 9   | **Capability (Cp/Cpk)**   | Set spec limits via inline inputs or SpecEditor → verify histogram + probability plot                             | Spec input UX (Target → expandable LSL/USL), Cp/Cpk calculation correctness              |
...
| 18  | **Factor management**     | Add factors mid-analysis via FactorManagerPopover → remove a factor → verify filter cleanup                       | State consistency, orphaned filters cleaned, drill path updated                          |
