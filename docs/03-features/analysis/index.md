---
tier: living
purpose: design
title: Analysis Features
audience: human
category: analysis
status: active
layer: L3
kind: infrastructure
serves:
  - docs/02-journeys/ia-nav-model.md
---

# Analysis Features

Statistical analysis capabilities and chart types.

- [ANOVA / Group-Mean Comparison](anova.md)
- [Boxplot](boxplot.md)
- [Capability Analysis](capability.md)
- [Capability Boxplot](capability-boxplot.md)
- [Capability Gap Trend Chart](capability-gap-trend-chart.md)
- [Capability Histogram](capability-histogram.md)
- [Characteristic Types](characteristic-types.md)
- [Defect Analysis](defect-analysis.md)
- [I-Chart](i-chart.md)
- [Nelson Rules](nelson-rules.md)
- [Pareto Chart](pareto.md)
- [Performance Mode](performance-mode.md)
- [Probability Plot](probability-plot.md)
- [ScatterFit / Regression Visualization](scatter-fit.md)
- [Staged Analysis](staged-analysis.md)
- [Statistics Panel](stats-panel.md)
- [Step Error Pareto](step-error-pareto.md)
- [Variation Decomposition](variation-decomposition.md)
