...
3. Algorithm runs: gap-ratio detection on sorted values + Anderson-Darling whole-sample pre-check + piecewise linear regression RSS reduction reported as confidence
...
**Algorithm choice rationale:** The algorithm runs in two stages. First, an Anderson-Darling whole-sample test gates detection — if your data is already approximately normal (p > 0.05), no inflection exists and the detection short-circuits. Second, gap-ratio detection scans the sorted values for the candidate breakpoint with the largest gap relative to the median between-value gap; ratios above 20× the median (empirically calibrated) become cuts. Piecewise linear regression on the prob plot's `(value, normalQuantile)` cloud reports the RSS reduction as user-facing confidence. KDE-valley detection answers "where's a gap in the density?"; change-point on sorted values answers "where does the sorted sequence shift its mean?" — neither is invariant to within-cluster spread, and neither produces a self-validating binning. Anderson-Darling p-value per segment is the stratification-validation signal the analyst reads downstream.
