            ...
            <div className="w-full max-w-2xl bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="p-6 border-b border-slate-700 bg-slate-800/50">
                    ...
                    <p className="text-slate-400 text-sm">
                        Confirm which columns to analyze from <strong>{datasetName}</strong>.
                ...
                <div className="p-6 space-y-8 bg-slate-900/30">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-3">
                            <div className="flex items-center justify-center w-6 h-6 rounded-full bg-blue-600 text-white text-xs font-bold">Y</div>
                            <h3 className="text-sm font-semibold text-slate-200 uppercase tracking-wide">Select Outcome (Measure)</h3>
                        </div>
                        <p className="text-xs text-slate-500 mb-3">Choose the numeric variable you want to improve (e.g., Weight, Diameter, Defect Count).</p>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto p-1 custom-scrollbar">
                            {availableColumns.map(col => (
                                <label
                                    key={`outcome-${col}`}
                                    className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${outcome === col
                                ...
                                >
                                    ...
                                    />
                                    <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${outcome === col ? 'border-blue-500 bg-blue-500' : 'border-slate-500'
                                        }`}>
                                        {outcome === col && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                                    </div>
                                    <span className={`text-sm font-medium ${outcome === col ? 'text-white' : 'text-slate-400'}`}>
                                        {col}
                                    </span>
                                </label>
                            ))}
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-3">
                            <div className="flex items-center justify-center w-6 h-6 rounded-full bg-emerald-600 text-white text-xs font-bold">X</div>
                            <h3 className="text-sm font-semibold text-slate-200 uppercase tracking-wide">Select Factors (Categories)</h3>
                            <span className="text-xs text-slate-500 ml-auto">{factors.length}/3 selected</span>
                        </div>
                        <p className="text-xs text-slate-500 mb-3">Choose up to 3 categorical variables to group by (e.g., Machine, Shift, Operator).</p>
                            ...
                            {availableColumns.map(col => {
                                ...
                                return (
                                    <button
                                        ...
                                        disabled={isOutcome}
                                        className={`flex items-center gap-3 p-3 rounded-lg border text-left transition-all ${factors.includes(col)
                                    ...
                                    >
                                        <div className={`w-4 h-4 rounded border flex items-center justify-center ${factors.includes(col) ? 'border-emerald-500 bg-emerald-500' : 'border-slate-500'
                                            }`}>
                                            {factors.includes(col) && <CheckSquare size={12} className="text-white" />}
                                        </div>
                                        <span className={`text-sm font-medium ${factors.includes(col) ? 'text-white' : 'text-slate-400'}`}>
                                            {col} {isOutcome && <span className="text-xs text-blue-400 ml-1">(Outcome)</span>}
                                        </span>
                                    </button>
                                );
                            })}
