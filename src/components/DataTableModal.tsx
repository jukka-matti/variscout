...
const DataTableModal = ({ isOpen, onClose }: DataTableModalProps) => {
    ...
    const getStatusColor = (value: any): string => {
        ...
        switch (status) {
            ...
            case 'FAIL_USL': return 'text-red-400';
            case 'FAIL_LSL': return 'text-amber-500';
                    ...
                    {localData.length === 0 ? (
                        ...
                        <table className="w-full border-collapse text-sm">
                            <thead className="sticky top-0 bg-slate-800 z-10">
                                <tr>
                                    <th className="px-3 py-2 text-left text-slate-400 font-semibold border-b border-slate-700 w-12">#</th>
                                    {columns.map(col => (
                                        <th
                                            key={col}
                                    ...
                                    <th className="px-3 py-2 text-center text-slate-400 font-semibold border-b border-slate-700 w-16">
                                                ...
                                                >
                                                    {editingCell?.row === absoluteIdx && editingCell?.col === col ? (
                                                        ...
                                                        <div className="px-2 py-1 cursor-pointer hover:bg-slate-700/50 rounded min-h-[28px]">
                                                            {row[col] !== '' && row[col] !== undefined ? String(row[col]) : <span className="text-slate-600">-</span>}
                                                        </div>
                                                    )}
                                                </td>
                                            ))}
