...
- Access-gating under V1 single-SKU (ADR-082): project-formal features are guarded by `canAccess(userId, members, action)` from `@variscout/core/projectMembership` (PR-WV1-1). Access-gate INSIDE the surface (Charter / Approach / Improve / Control / Report writes), not at surface entry — Sponsor role gets Report-only view; Member + Lead get edit; non-members never see the project in their list.
