...
import type { UseDefectEvidenceMapResult } from '@variscout/hooks';
import {
  NodeContextMenu,
  EdgeContextMenu,
  EdgeDetailCard,
...
interface InvestigationMapViewProps {
  ...
  /** Defect Evidence Map view state (only provided when analysisMode === 'defect') */
  defectMapView?: string;
  onDefectMapViewChange?: (view: string) => void;
...
export const InvestigationMapView: React.FC<InvestigationMapViewProps> = ({
  ...
  // Determine what to render in the map area
  const showCrossType = defectMapView === 'cross-type' && defectEvidenceMap?.crossTypeMatrix;
  const showInsufficient =
    defectEvidenceMap?.insufficient && defectMapView !== 'all' && defectMapView !== 'cross-type';

  return (
    <div className="flex-1 min-h-0 flex flex-col bg-surface-secondary">
      {/* Defect type selector (only in defect mode) */}
      {defectMapView !== undefined && onDefectMapViewChange && defectEvidenceMap && (
        <div className="flex-none px-3 py-2 border-b border-edge">
          <DefectTypeSelector
            selectedView={defectMapView}
            onViewChange={onDefectMapViewChange}
