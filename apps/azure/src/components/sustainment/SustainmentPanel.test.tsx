...
beforeEach(() => {
  ...
  vi.mocked(azureHubRepository.sustainmentRecords.listByHub).mockResolvedValue([]);
  vi.mocked(azureHubRepository.sustainmentReviews.listByRecord).mockResolvedValue([]);
});

describe('SustainmentPanel (Azure)', () => {
  ...
  it('selects an existing live record, reads reviews through the repository, and persists edits by dispatch', async () => {
    ...
    vi.mocked(azureHubRepository.sustainmentRecords.listByHub).mockResolvedValue([record]);
    vi.mocked(azureHubRepository.sustainmentReviews.listByRecord).mockResolvedValue([
      {
        id: 'review-1',
        recordId: 'sr-1',
