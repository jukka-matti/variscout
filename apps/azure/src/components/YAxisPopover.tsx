    ...
    >
      ...
      <div className="p-3 space-y-3">
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-[10px] text-slate-500">Auto: {autoMax.toFixed(2)}</span>
          </div>
          <input
            type="number"
            step="any"
            ...
            className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-sm text-white text-right outline-none focus:border-blue-500 transition-colors"
          />
        </div>
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="text-[10px] text-slate-400 uppercase">Min</label>
            <span className="text-[10px] text-slate-500">Auto: {autoMin.toFixed(2)}</span>
          </div>
          <input
            type="number"
            step="any"
      ...
      <div className="px-3 pb-3 space-y-2">
        ...
        {!isAutoScale && (
          <button
            onClick={handleReset}
            className="w-full py-1.5 rounded text-xs font-medium transition-colors flex items-center justify-center gap-1 bg-slate-700 hover:bg-slate-600 text-slate-400 hover:text-white"
          >
            <RotateCcw size={12} />
            Reset to Auto
          </button>
