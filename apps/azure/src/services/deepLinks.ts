...
export type DeepLinkMode = 'home' | 'report' | 'improvement' | 'analyze';
...
const VALID_MODES = new Set<string>(['home', 'dashboard', 'report', 'improvement', 'analyze']);
...
export function parseDeepLink(search: string): DeepLinkParams {
  ...
  const mode =
    modeRaw && VALID_MODES.has(modeRaw)
      ? ((modeRaw === 'dashboard' ? 'home' : modeRaw) as DeepLinkMode)
      : null;
