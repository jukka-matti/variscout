...
describe('useActiveIPContext', () => {
  ...
  it('returns Azure user scope and active IP when stored', () => {
    useActiveIPStore
      .getState()
  ...
  it('falls back to local user id when Azure user id is unavailable', () => {
    const { result } = renderHook(() => useActiveIPContext(baseHub, null));
  ...
  it('clears active state when the stored IP is not live on the hub', () => {
    useActiveIPStore
      .getState()
      .setActiveIP({ hubId: 'hub-1', userId: 'mira@example.com' }, 'missing', 123);

    const { result } = renderHook(() => useActiveIPContext(twoProjectHub, 'mira@example.com'));
