  ...
  const SpecRow = ({
    ...
    <div className="flex items-center gap-2">
      <label className="flex items-center gap-2 flex-1 cursor-pointer">
        ...
        />
        <span className="text-sm text-content w-14">{label}</span>
      </label>
      <input
        type="number"
        step="any"
        ...
        className={`w-24 bg-surface border border-edge rounded px-2 py-1.5 text-sm text-white text-right outline-none focus:border-blue-500 transition-colors ${
    ...
    <div className={`relative ${className}`}>
      ...
      <button
        ...
        className={`flex items-center gap-1 text-xs px-2 py-1 rounded transition-colors ${
          hasAnySpec
            ? 'text-blue-400 hover:text-blue-300 hover:bg-surface-tertiary/50'
      ...
      {isOpen && (
        <div
          ref={popoverRef}
          className="absolute top-full left-0 mt-2 w-64 bg-surface-secondary border border-edge rounded-lg shadow-2xl z-50 animate-fade-in"
        >
          {/* Header */}
