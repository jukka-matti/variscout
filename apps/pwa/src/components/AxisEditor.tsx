...
const AxisEditor = ({
  ...
  return (
    <div
      className="absolute z-50 bg-surface-secondary border border-edge-secondary rounded-lg shadow-xl p-3 flex flex-col gap-3 w-64"
      style={style}
    >
      ...
      <div className="space-y-3 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
        {/* Main Axis Name */}
        <div>
          <label className="text-[10px] text-content-secondary uppercase block mb-1">
            Display Name
          </label>
          <div className="relative">
            <Type size={12} className="absolute left-2 top-2 text-content-muted" />
            <input
              type="text"
              value={newAlias}
          ...
          {originalName !== newAlias && (
            <div className="text-[10px] text-content-muted mt-1 pl-1">Original: {originalName}</div>
          )}
        </div>
            ...
            <div className="space-y-1">
              {values.map(val => (
                <div key={val} className="flex items-center gap-2">
                  <span className="text-[10px] text-content-muted w-1/3 truncate" title={val}>
                    {val}
                  </span>
