...
- **NEXT = Task 3** (clear-scope-on-IP-switch hook): create `packages/hooks/src/useClearScopeOnIPSwitch.ts(activeIPId, clearFn)` — ref-guard, fire `clearFn` ONLY on genuine non-null A→B (A!==B); NOT on first render / null→id activation / id→null. Use existing `useAnalysisScopeStore.clearScope` (analysisScopeStore.ts:110). Barrel-export from `packages/hooks/src/index.ts` (no exports/tsconfig#paths change). Wire in Azure at `Editor.tsx` right after L633. Then 4(PWA mirror, depends on 3)→5→6→7→8→final.
