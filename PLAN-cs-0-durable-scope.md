...
- **NEXT = Task 4** (PWA mirror): wire `useClearScopeOnIPSwitch(activeIPContext.activeIP?.id ?? null, useAnalysisScopeStore(s=>s.clearScope))` in `apps/pwa/src/App.tsx` `AppMain` right after the `useActiveIPContext(sessionHub)` call (~L174). Add imports: `useClearScopeOnIPSwitch` (hooks block ~L45-52), `useAnalysisScopeStore` (stores block ~L54-64, currently absent). Behavior is covered by the Task 3 hook unit test + final E2E — wiring is tsc-verified, no brittle full-shell render test. Then 5(Azure seed Y)→6(PWA seed Y)→7(Finding.scopeId Azure)→8(PWA)→final.
